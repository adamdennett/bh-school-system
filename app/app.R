...
server <- function(input, output, session) {
  ...
  output$caveats <- renderUI(HTML(paste0(
    ...
    "<p><b>Children cannot leave the city in the model.</b> It places every ",
    "Brighton child in a Brighton school, so every school's intake is slightly ",
    "high. The Catchments tab does show children offered a place outside the ",
    "city, but that figure is not modelled: it is the Schools Adjudicator's ",
