...
**The east is now served from the east.** Of the `r n_east`
easternmost neighbourhoods, `r sum(ec210$assigned == "Longhill")` are in
Longhill's catchment and `r sum(ec210$assigned == "BACA")` in BACA's,
both eastern schools. The mean journey to an assigned catchment school
is `r sprintf("%.0f", weighted.mean(ec210$t_assigned, ec210$Oi))` minutes
against `r sprintf("%.0f", weighted.mean(ec210$t_near, ec210$Oi))` if
every one went to whichever it could reach quickest --- a penalty of
about `r sprintf("%.0f", weighted.mean(ec210$t_assigned, ec210$Oi) - weighted.mean(ec210$t_near, ec210$Oi))`
minutes. Under the flow-seeded version it was twenty.

The `r sum(ec210$assigned == "DS_Varndean")` that still go west are the
far coastal strip, `r sprintf("%.0f", min(ec210$t_near[ec210$assigned == "DS_Varndean"]))`
to `r sprintf("%.0f", max(ec210$t_near[ec210$assigned == "DS_Varndean"]))`
minutes from Elm Grove even at their nearest. They are a long way from
every school in the city, and no boundary changes that.

### What the two admission numbers actually decide {#sec-fr-elm-pan}
...
**Raising the admission number from `r sw$pan[1]` to 210 barely changes
the map and changes the arithmetic a great deal.** One more
neighbourhood joins Longhill's catchment. But the capacity gap falls
from `r sprintf("%+.0f%%", 100 * sw$longhill_gap[sw$pan == 150])` to
`r sprintf("%+.0f%%", 100 * sw$longhill_gap[sw$pan == 210])`, because at
Elm Grove **Longhill is the nearest school for far more of the city than
it has places for**.

That is the real result of moving it. A site in the middle of Brighton
is close to a great many children; a site at Ovingdean is close to few.
The sweep says the area Longhill would be nearest to at Elm Grove needs
about `r sw_bal$pan[1]` places to balance ---
`r sprintf("%+.0f%%", 100 * sw_bal$longhill_gap[1])` at that number ---
which is larger than Longhill has been at any point in this document.

::: {.callout-important appearance="simple"}
## Reducing Stringer's admission number would not fix this

It is the obvious lever and it does not reach. Targets are shares of the
city's places, so taking 120 places off the Stringer/Varndean pair moves
Longhill's target by about **eleven children**. The binding constraint
is Longhill's own admission number, not its neighbour's.

The choice the relocation actually presents is between three things,
and only two of them can hold at once:

- **A school of 150 or 210 places at Elm Grove.**
- **A catchment drawn round the children it is nearest to.**
- **A capacity balance across the city.**

At `r sw$pan[1]` places the first two force a gap of
`r sprintf("%+.0f%%", 100 * sw$longhill_gap[sw$pan == 150])`. Keeping the
balance instead means handing the east to schools it is not nearest to,
which is what the flow-seeded version did. Only a substantially larger
school at Elm Grove holds all three, and that is a different proposal
from the one on the table.
:::
...
So the relocation that @sec-longhill finds best for Longhill is not the
redesign that is best for the city, on any measure here except the
capacity gap it creates for itself. That is not an argument against
