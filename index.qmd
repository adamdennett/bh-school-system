...
`r sprintf("%.1f", co("elm", "dep_gap"))` minutes; with CoMArt open, to
...
1. **Make a smaller Longhill at the top of Elm Grove the lead option, and
   ...
   `r sprintf("%.1f", co("elm", "dep_gap"))` minutes against
...
5. **Trim Stringer, Varndean and Blatchington Mill only if Longhill stays
   at Ovingdean** --- to 300, 270 and 300, for the next two or three
   rounds while they are full, and only with the Whitehawk redraw. With
...
deprived-journey gap of `r sprintf("%.1f", co(pk, "dep_gap"))` minutes
...
The Elm Grove runs carry two further limits, found while checking the
simulator: the competition term still measures distances from Longhill's
Ovingdean site, and children refused elsewhere are re-offered places
using the second preferences families gave with Longhill at Ovingdean.
Both are likely to understate how many children a school at Elm Grove
would draw, and both should be corrected before the move is costed.
