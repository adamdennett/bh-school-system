...
Longhill's catchment raises segregation a little, but not reliably ---
the box under Priority 2 sets out why earlier versions said otherwise.
Priority 6 makes no measurable difference. Closing Longhill, removing
the oversubscription priorities and trimming Cardinal Newman and King's
lower segregation; the power diagram, Elm Grove and the packages raise
it. Most options differ by a few thousandths on the index, and the
Whitehawk box shows a change of measure moving results of that size, so
only the larger differences should be read as findings. The journey, displacement, catchment and
...
- **Whitehawk back in Longhill's catchment** adds
  ...
  shows it lowers displacement. Its effect on segregation is small and
  not reliably in either direction (Priority 2).
...
**Returning Whitehawk to Longhill's catchment makes little difference to
segregation, and not reliably in either direction.** In 2030 the index
is `r sprintf("%.3f", co("today", "gorard"))` today and
`r sprintf("%.3f", co("whitehawk", "gorard"))` with the redraw. Earlier
versions of this section found first that it lowered segregation and
then that it clearly raised it. How far it moves the index, and which
way, turns on how disadvantage is counted, and the box below sets out
why.
...
`r fmt_pct(100 * wh_s("Varndean", "whitehawk", "share"), 1)`, closer to
it, which takes from it. Stringer's goes from
`r fmt_pct(100 * wh_s("Dorothy Stringer", "today", "share"), 1)` to
`r fmt_pct(100 * wh_s("Dorothy Stringer", "whitehawk", "share"), 1)`,
further below it, which adds. On balance the index is
...
`r if (length(unique(sign(round(wh_chg, 3)))) > 1) "The sign changes with the strength of the pull. The honest reading is that the redraw's effect on segregation is too small for this model to call." else "The sign is the same in every version, but the size is small beside the differences between the measures."`
...
**What that leaves.** The case for or against returning Whitehawk rests
on other things: a shorter journey for the children who would take a
...
- *Whitehawk is not the trade-off it looked.* Returning Whitehawk to Longhill's
  catchment gives Longhill
  `r fmt_n(co("whitehawk", "lh_intake") - co("today", "lh_intake"))` more
  children a year by 2030 and lowers
  displacement, and its effect on how mixed intakes are is small and
  turns on how disadvantage is counted. It is not the clear trade
  against segregation earlier versions of this section showed.
...
2. **If the site cannot be had, return Whitehawk to Longhill's catchment
   ...
   children a year by 2030 and lowers displacement. Its effect on how
   mixed intakes are is small and not reliably in either direction, and
   it does little for Whitehawk's journeys while the school is at
   Ovingdean.
