...
```{r fig-idaci-profiles}
#| fig-cap: "Households with dependent children by IDACI decile, in each region of each design. Decile 1 is the most deprived tenth of neighbourhoods nationally. A design in which every panel had the same shape would be one where no catchment concentrated deprivation."
#| fig-height: 8.5
#| column: page

ip <- fr$idaci_profiles %>%
  mutate(design = factor(design, fr_designs),
         lab = ifelse(grp %in% names(CATCH_FROM_MODEL),
                      unname(CATCH_LABELS[CATCH_FROM_MODEL[grp]]),
                      short_sch(grp)))

ggplot(ip, aes(factor(idaci_decile), hh, fill = idaci_decile)) +
  geom_col() +
  facet_grid(design ~ lab, scales = "free_y",
             labeller = labeller(design = label_wrap_gen(22),
                                 lab = label_wrap_gen(14))) +
  scale_fill_gradientn(colours = rev(RColorBrewer::brewer.pal(11, "RdYlBu")),
                       limits = c(1, 10), breaks = c(1, 5, 10),
                       labels = c("1 most\ndeprived", "5", "10 least"),
                       name = "IDACI decile") +
  labs(x = "IDACI decile", y = "Households with dependent children",
       title = "Deprivation profile of each catchment, under each design",
       subtitle = "The consultation's own method: postcodes inside a catchment boundary, households with dependent children by decile.",
...
```{r tbl-idaci-spread}
#| tbl-cap: "The share of each region's households with dependent children that sit in the four most deprived deciles nationally, for the least and most deprived region of each design. The spread between them is the measure of how much the map concentrates deprivation."

fr$idaci_summary %>%
  mutate(design = factor(design, fr_designs)) %>%
  arrange(design) %>%
  transmute(Design = design,
            `Least deprived region` = fmt_pct(100 * lo, 0),
            `Most deprived region` = fmt_pct(100 * hi, 0),
...
**Every alternative is less segregated than the map in force.** Under
the current catchments the most deprived region has
`r fmt_pct(100 * i_now$hi, 0)` of its households with dependent children
in the four most deprived deciles and the least deprived has
`r fmt_pct(100 * i_now$lo, 0)` --- a spread of
`r sprintf("%.0f", 100 * i_now$spread)` percentage points. The power
diagram narrows that to `r sprintf("%.0f", 100 * i_pd$spread)` and the
flow regions to `r sprintf("%.0f", 100 * i_pair$spread)`.

The gain comes from the same place in both: **the current map's most
deprived catchment is more deprived than any alternative's**, falling
from `r fmt_pct(100 * i_now$hi, 0)` to
`r fmt_pct(100 * i_pd$hi, 0)` under the power diagram. Neither design
sets out to reduce segregation. Both do, because the current boundaries
follow lines that separate the east of the city from the rest and any
redrawing that balances capacity has to cut across them.

That said, `r sprintf("%.0f", 100 * i_pd$spread)` points is still a very
wide spread. **No redrawing of a catchment map fixes residential
segregation**, because the segregation is in where people live, not in
where the lines are. What a map can do is stop amplifying it.
...
And on deprivation it is the **weakest of the alternatives**: a spread
of `r sprintf("%.0f", 100 * i_elm$spread)` points against
`r sprintf("%.0f", 100 * i_pair$spread)` for the same method at
Ovingdean. Moving the school west moves its catchment west, into the
part of the city the current map already separates out, and the new
boundary reproduces much of that separation.
...
**And the headline is a negative one.** The best design on offer moves
between a sixth and a third of the cohort, improves capacity balance,
narrows the deprivation spread by a few points --- and changes how many
children get their first choice by nothing at all. Redrawing the map is
worth doing on its own terms. It is not an answer to the problem the
