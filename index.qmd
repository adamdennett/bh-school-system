...
## Try the options yourself

Everything in this document comes out of one model, and that model runs in a
browser. **[Open the school places simulator](https://adamdennett-bh-school-system.share.connect.posit.cloud/){target="_blank"}**
to change admission numbers, catchments, Longhill's site or the
oversubscription priorities, and watch what happens to each school's intake,
to journey times, to the mix of pupils and to school budgets.

There is also [a guide to the model, stage by
stage](model-guide.qmd), for anyone who wants to check how it works.
:::
...
# How strongly demand follows the headline score, and where the two
# schools serving the most disadvantaged intakes sit on each measure.
ex_att8_pct <- 100 * (exp(ch_one(CH_DEFAULT, "att8")$slope) - 1)
ex_cd <- cd %>% mutate(r_att8 = rank(-att8), r_va = rank(-va))
ex_cd_of <- function(s, col) ex_cd[[col]][ex_cd$short == s]
...
```{r exec-va}
#| include: false
ex_vac <- bh_data("value_added_choice.rds")
ex_va_sch <- function(s, col) {
  d <- ex_vac$schools[ex_vac$schools$year == 2026, ]
  d[[col]][d$short == s]
}
ex_va_even <- ex_vac$even[ex_vac$even$year == 2026, ]
ex_va_city <- function(b, col)
  ex_vac$city[[col]][ex_vac$city$year == 2026 & ex_vac$city$basis == b]
```
...
**For disadvantaged pupils, concentration is not the harm it is assumed
to be.** Nationally, once absence and prior attainment are accounted
for, disadvantaged pupils do slightly *better* in schools with more
disadvantaged pupils, not worse. The effect is small, and it appears to
run through schools organised to serve those pupils rather than through
concentration itself --- but it points the opposite way to the
council's aim of reducing concentrations. Brighton Aldridge is the local
example: about half its pupils are disadvantaged, and its disadvantaged
pupils leave with better GCSE results than comparable pupils elsewhere
in England (@sec-concentration-evidence). So where an option here raises
the segregation index, that counts against it under the council's
policy aim; it is not, on the evidence, a cost to disadvantaged pupils'
results.
...
**Families are choosing on the wrong number, and the council's own
advice points them to it.** Its secondary admissions guide tells parents
to look in each school's prospectus for its "National Curriculum test
results, public exam results" and to read its Ofsted reports
([Secondary school admissions guide 2027 to 2028](https://www.brighton-hove.gov.uk/schools-and-learning/school-policies-reports-strategies-and-other-documents/secondary-school-admissions-guide-2027-2028)) ---
raw figures that mostly describe who a school admits. Families do as
they are advised. Headline Attainment 8 explains
`r fmt_pct(100 * ex_r2_att8, 0)` of the variation in how heavily each
school is preferred, and each point of it goes with about
`r sprintf("%.0f", ex_att8_pct)`% more preferences per place. The
value-added measure --- what the school actually contributes ---
explains `r fmt_pct(100 * ex_r2_va, 0)`.

**That advice works against the council's own position.** A school
serving a more disadvantaged intake posts a lower headline score, so it
is chosen less; its intake shrinks and becomes more disadvantaged still;
and its headline score falls again. Brighton Aldridge is last of the ten
on the headline measure and
`r c("first","second","third","fourth","fifth","sixth","seventh","eighth","ninth","tenth")[ex_cd_of("Brighton Aldridge", "r_va")]`
on value added. Longhill's headline score is
`r sprintf("%.0f", max(ex_cd$att8) - ex_cd_of("Longhill", "att8"))` points
below the city's highest, but on value added it is
`r sprintf("%.1f", max(ex_cd$va) - ex_cd_of("Longhill", "va"))` points
below the best --- most of what separates it from the most popular
schools is intake, not teaching. By steering families towards the
headline number, the council lowers the pull of the very school whose
viability it is trying to secure, shrinks the roll its funding depends
on, and concentrates disadvantage in the schools its own guidance makes
least attractive --- the opposite of its stated aim. It is the one part
of the Longhill problem the council could change at no cost
(@sec-choice).

**And it is the largest lever modelled here.** Run the city with families
reading the four-year value-added measure as closely as they now read the
headline score, holding everything else --- same places, same catchments,
same rules --- and the intakes move further than any boundary change in
this document. Brighton Aldridge goes from
`r fmt_n(round(ex_va_sch("Brighton Aldridge", "att8_intake")))` children
to `r fmt_n(round(ex_va_sch("Brighton Aldridge", "va_intake")))` on 180
places, from half empty to
`r sprintf("%.0f%%", 100 * ex_va_sch("Brighton Aldridge", "va_fill"))`
full; Hove Park and Portslade Aldridge fill; mean journeys shorten, and
the extra distance the most deprived children travel falls from
`r sprintf("%.1f", ex_va_city("Attainment 8", "dep_gap"))` to
`r sprintf("%.1f", ex_va_city("Value added", "dep_gap"))` minutes each
way. **The share of the city's empty Year 7 places sitting in just three
schools falls from
`r sprintf("%.0f%%", 100 * ex_va_even$top3_att8)` to
`r sprintf("%.0f%%", 100 * ex_va_even$top3_va)`** --- which is the
difference between a surplus that is a budgeting problem and one that is
a viability problem. It creates no places and removes none: what changes
is which schools are empty (@sec-value-added-choice).
...
2. **Treat the instruments as a package.** Moving the school, changing
   ...
   available option. If the council puts access for disadvantaged
   children first, the package should be led by a smaller Longhill at
   the top of Elm Grove with the catchments redrawn round it
   (@sec-weighing).
3. **Change the admissions guide's advice, and publish a contextualised
   measure alongside Attainment 8.** It costs nothing, needs no
   consultation, and is the largest single change modelled in this
   document: if families read what a school adds rather than the score it
   publishes, Brighton Aldridge fills
   (`r fmt_n(round(ex_va_sch("Brighton Aldridge", "att8_intake")))` children
   to `r fmt_n(round(ex_va_sch("Brighton Aldridge", "va_intake")))` on 180
   places), the share of the city's empty places concentrated in three
   schools falls from
   `r sprintf("%.0f%%", 100 * ex_va_even$top3_att8)` to
   `r sprintf("%.0f%%", 100 * ex_va_even$top3_va)`, and deprived children's
   extra travel falls by about a minute each way (@sec-value-added-choice).
...
5. **Release ten tables** (§9) that would convert most of the bands in
...
One of the ten is not a single building. **Hove Park teaches Years 7 and
8 at its Valley Campus on Hangleton Way**, 1.65 km west of the Nevill
Road site its address gives, and moves pupils to Nevill Road in Year 9.
The council gave statutory notice on 8 June 2026 to close the Valley
Campus on 31 August 2028 and bring the school onto one site. Because
this document is about Year 7, every figure here for entry years up to
2027 puts Hove Park at Hangleton Way and every figure from 2028 puts it
at Nevill Road. It makes little difference to how many children the
school takes, and a large difference to which children: at Hangleton Way
it draws about twice as many from Hangleton & Knoll and much less from
Westdene and Poets' Corner
([the model guide](https://adamdennett.github.io/bh-school-system/model-guide.html#sec-cij-hove-park)).
...
### For disadvantaged pupils, the direction reverses {#sec-concentration-evidence}

The fall in Attainment 8 that goes with a higher disadvantaged share is
an average over all pupils. [How to Pull the Right Lever](https://adamdennett.github.io/school_attainment_tool/index.html)
fits the same model separately for disadvantaged and non-disadvantaged
pupils, and the two disagree about direction.

- **For non-disadvantaged pupils** a school with more disadvantaged
  pupils is associated with lower attainment, even after their own
  attendance and prior attainment are accounted for.
- **For disadvantaged pupils** it is associated with slightly *higher*
  attainment, once absence and low prior attainment are accounted for.
  The report finds this in the four-year panel and in each year
  separately, and under different measures of disadvantaged attainment
  and of school disadvantage ([robustness checks](https://adamdennett.github.io/school_attainment_tool/model_experiments.html#sec-directionality),
  [model results](https://adamdennett.github.io/school_attainment_tool/model_results.html#disadvantaged-pupils)). It is strongest in the non-GCSE
  qualifications of the Attainment 8 'open' element.
- **The effect is small, and it is mostly about the schools.** In the
  full multilevel model the coefficient shrinks towards zero once region,
  local authority and Ofsted rating are allowed for. The report's reading
  is that disadvantaged pupils do not gain simply from being together;
  they gain because schools with many of them --- in some places and with
  some inspection records --- have learned to serve them well, including
  through well-targeted vocational routes.

The report is explicit that this is **not an argument for concentrating
disadvantaged pupils**, and that mixed intakes may have benefits it does
not measure. Its argument is narrower: where a city's geography already
concentrates disadvantage in some schools, deconcentration pursued at
the cost of longer journeys is unlikely to help the pupils it is meant to
help, and may harm them.

::: {.callout-note appearance="simple"}
## Brighton Aldridge is the local example

Brighton Aldridge Community Academy has about half its pupils
disadvantaged, some of the city's highest absence, and around 30% of its
pupils arriving with low prior attainment. On the same panel model's
contextualised results, a disadvantaged pupil leaving BACA in 2024--25
did about **5.7 Attainment 8 points better** than a comparable pupil at a
similar school elsewhere in England, and about **3.1 points better on
average over four years** --- the strongest result in the city for
disadvantaged pupils, and far larger than the fraction of a point that
changing a school's disadvantage mix would be expected to deliver. Ofsted
rated the school Good in every area in April 2025.

@sec-noise applies to BACA as to any school: a single year of
disadvantaged results is one draw, and the four-year figure is the one to
lean on. It is still well above anything the redistribution of pupils
could deliver.
:::

**What follows for this document.** Several options in @sec-weighing ---
Longhill at Elm Grove, Whitehawk back in Longhill's catchment --- raise
the segregation index because they put more disadvantaged children in
the same school. The council's policy aim treats that as a cost. The
national evidence does not: provided the school is organised and funded
for the intake it gets, a more concentrated intake is not a worse one for
its disadvantaged pupils. The segregation index is reported here because
the council asks for it, not because it measures harm.
...
meant to widen access. @sec-fr-p6 finds that, in the full model, it
makes almost no difference to how mixed the city's intakes are.
...
data: parents are told to look in each school's prospectus for its
"National Curriculum test results, public exam results" and to read
its Ofsted reports ([Secondary school admissions guide 2027 to
2028](https://www.brighton-hove.gov.uk/schools-and-learning/school-policies-reports-strategies-and-other-documents/secondary-school-admissions-guide-2027-2028)). Nothing in that advice adjusts for who a school
admits. It is worth asking which published number families respond to.
...
## Why Hove Park takes more disadvantaged children than Blatchington Mill {#sec-hove-park-mix}

```{r hp-campus-prep}
#| include: false
hpc <- bh_data("hove_park_campus.rds")
hp_g <- function(case, col) hpc$gap[[col]][hpc$gap$case == case]
hp_m <- function(year, col) hpc$move[[col]][hpc$move$year == year]
hp_lam <- function(s) hpc$lambda[[s]]
hp_pts <- function(x) sprintf("%.1f", 100 * x)
# The attractiveness multiplier the move is worth, in Attainment 8
# points, using this document's own choice fit (section 5.4) so the two
# cannot drift apart.
hp_att8 <- log(hpc$mult) / ch_one(CH_DEFAULT, "att8")$slope
hp_one_pt <- exp(ch_one(CH_DEFAULT, "att8")$slope)
hp_att8_now <- cd$att8[cd$school == "Hove Park School"]
hp_att8_top <- max(cd$att8)
```

This question comes up repeatedly, and it is a good one. Hove Park and
Blatchington Mill share a catchment and stand
`r sprintf("%.1f", hpc$km_nevill)` km apart, yet Hove Park's Year 7 is
about `r hp_pts(hp_g("As calibrated", "hove_park"))`% disadvantaged
against Blatchington Mill's
`r hp_pts(hp_g("As calibrated", "blatchington"))`% --- a gap of
`r hp_pts(hp_g("As calibrated", "gap"))` points between two schools with
the same catchment and, on the face of it, the same geography.

The model can separate two explanations that are usually run together.
The disadvantage layer has a term for each school, $\lambda_j$, which is
how strongly that school pulls the disadvantaged children of a
neighbourhood it already draws from. Set every $\lambda$ to one and each
school simply takes its proportional share of each neighbourhood's
disadvantaged children: what is left is **geography**. The difference
between that and the published shares is **sorting within
neighbourhoods**.

```{r hp-campus-gap}
#| tbl-cap: "The disadvantaged share of each school's modelled Year 7, 2026, under four treatments."
hpc$gap %>%
  transmute(Treatment = case,
            `Hove Park` = paste0(hp_pts(hove_park), "%"),
            `Blatchington Mill` = paste0(hp_pts(blatchington), "%"),
            Gap = sprintf("%+.1f pts", 100 * gap)) %>%
  knitr::kable(align = "lrrr")
```

Geography explains about
`r sprintf("%.0f", 100 * hp_g("Geography only", "gap") / hp_g("As calibrated", "gap"))`%
of the gap --- roughly a third --- and all of that comes from one thing:
**Year 7 is not taught at Hove Park's registered address.** It is taught
at the Valley Campus on Hangleton Way, `r sprintf("%.1f", hpc$km_valley)`
km from Blatchington Mill and in Hangleton rather than central Hove. Put
Year 7 at Nevill Road and the geographic part of the gap disappears
entirely: `r sprintf("%+.1f", 100 * hp_g("Geography only, Nevill Road", "gap"))`
points, which is nothing. Two schools half a kilometre apart draw the
same children, as you would expect.

The other two thirds is sorting, and the fitted $\lambda$ is a
measurement of it:

```{r hp-campus-lambda}
#| tbl-cap: "How strongly each school draws the disadvantaged children of the neighbourhoods it already serves. One means its proportional share."
tibble::tibble(School = short_sch(names(hpc$lambda)),
               `Pull on disadvantaged children` = sprintf("%.2f", hpc$lambda)) %>%
  arrange(desc(as.numeric(`Pull on disadvantaged children`))) %>%
  knitr::kable(align = "lr")
```

Hove Park's `r sprintf("%.2f", hp_lam("Hove Park School"))` against
Blatchington Mill's `r sprintf("%.2f", hp_lam("Blatchington Mill School"))`
is the heart of it: from the same streets, Hove Park takes about
`r sprintf("%.0f", 100 * (hp_lam("Hove Park School") / hp_lam("Blatchington Mill School") - 1))`%
more of the disadvantaged children than its share of the intake implies.
The two church schools sit at the other end ---
`r sprintf("%.2f", hp_lam("King's School"))` and
`r sprintf("%.2f", hp_lam("Cardinal Newman Catholic School"))`, so about
half their proportional share --- as do Varndean and Dorothy Stringer.
Longhill is off the scale in the other direction.

That is the interaction worth naming. Closing King's and Cardinal
Newman in the model narrows the geography-only gap from
`r hp_pts(hp_g("Geography only", "gap"))` points to
`r hp_pts(hp_g("Geography only, no faith schools", "gap"))`, so their
pull is real but modest as a share of the whole. The larger effect is
not that the church schools take children out of Hove Park's catchment;
it is that **across the city the schools families compete hardest to
enter take a smaller share of the disadvantaged children from every
neighbourhood they serve, and the undersubscribed schools take the
rest.** Blatchington Mill fills; Hove Park takes those who did not get
in, and those whose families did not name it first.

### Will it even out when the Valley Campus closes? {#sec-hove-park-even}

This is the obvious follow-up, and it is a good theory: if the campus is
what puts Hove Park's Year 7 in Hangleton, bringing everyone to Nevill
Road should bring the two schools together. The model's answer is **half
right**.

```{r hp-campus-even}
#| tbl-cap: "The gap between the two schools' modelled Year 7, with Hove Park's Year 7 at each campus."
hpc$move %>%
  filter(year %in% c(2026, 2028, 2030)) %>%
  transmute(`Entry year` = year,
            `Hove Park` = sprintf("%.1f%% → %.1f%%", 100 * valley_dep, 100 * nevill_dep),
            `Blatchington Mill` = sprintf("%.1f%% → %.1f%%",
                                          100 * valley_dep_bm, 100 * nevill_dep_bm),
            `Gap` = sprintf("%.1f → %.1f pts", 100 * valley_gap, 100 * nevill_gap)) %>%
  knitr::kable(align = "lrrr")
```

The gap narrows from
`r sprintf("%.1f", 100 * hp_m(2026, "valley_gap"))` points to
`r sprintf("%.1f", 100 * hp_m(2028, "nevill_gap"))` --- about
`r sprintf("%.0f", 100 * (hp_m(2026, "valley_gap") - hp_m(2028, "nevill_gap")) / hp_m(2026, "valley_gap"))`%
of it --- and it closes from both ends, Hove Park falling and
Blatchington Mill rising as the two swap children. But it does not
disappear. Hove Park's intake stays around
`r sprintf("%.0f", 100 * hp_m(2028, "nevill_gap"))` points more
disadvantaged, because the part of the gap that is sorting rather than
geography does not care where the building is. Moving a school changes
which streets it draws from; it does not change which families name it
first, or which school fills and which takes those who did not get in.

One caveat runs the other way, and it matters. $\lambda$ is fitted over
years when the school was split across two sites. If the split site is
itself part of why families sort as they do --- a lower school in
Hangleton shaping what parents think they are choosing --- then some of
what the model counts as sorting would move with the building too, and
more of the gap would close. The model holds that fixed by construction,
so **`r sprintf("%.0f", 100 * (hp_m(2026, "valley_gap") - hp_m(2028, "nevill_gap")) / hp_m(2026, "valley_gap"))`%
is a floor, not a ceiling.** Which of the two readings is right will be
visible in the September 2028 and 2029 intakes.

::: {.callout-warning}
## What this is, and what it is not

$\lambda_j$ is **fitted** to reproduce each school's published
disadvantaged share. It measures sorting; it does not explain it. The
model can say how much of the Hove Park--Blatchington gap geography
accounts for --- about a third, all of it the campus --- and how much it
cannot. It cannot say whether the remaining two thirds is parental
choice, the order preferences are given in, the lottery, the priorities,
or something else, and nothing here should be read as saying which.
:::

## Does moving the school raise demand, or does the school have to change? {#sec-hove-park-demand}

The consolidation on Nevill Road in September 2028 is as close to a
natural experiment as this system offers: one school, one known date,
one move of `r sprintf("%.2f", hpc$km_move)`
km east, with everything else held. The model's answer:

```{r hp-campus-move}
#| tbl-cap: "Hove Park's modelled Year 7 with the entry year taught at each campus, and how disadvantaged that intake is."
hpc$move %>%
  transmute(`Entry year` = year,
            `At the Valley Campus` = sprintf("%.0f", valley),
            `At Nevill Road` = sprintf("%.0f", nevill),
            Difference = sprintf("%+.0f", nevill - valley),
            `Disadvantaged share` = sprintf("%.0f%% → %.0f%%",
                                            100 * valley_dep, 100 * nevill_dep)) %>%
  knitr::kable(align = "lrrrr")
```

The move is worth about
**`r sprintf("%+.0f", hp_m(2028, "nevill") - hp_m(2028, "valley"))` children
a year**, an intake `r sprintf("%.0f", 100 * (hp_m(2028, "nevill") / hp_m(2028, "valley") - 1))`%
larger, and it holds as the cohort shrinks. It is not nothing: it is
most of the difference between a viable form entry and an empty one.

But set it against the other lever. Staying at Hangleton Way and making
the school **`r sprintf("%.0f", 100 * (hpc$mult - 1))`% more attractive**
produces exactly the same intake. On this document's own choice fit
(@sec-choice), that multiplier is worth about
**`r sprintf("%.1f", hp_att8)` Attainment 8 points**. Hove Park's
Attainment 8 is `r sprintf("%.1f", hp_att8_now)`; the highest in the city
is `r sprintf("%.1f", hp_att8_top)`. So the whole benefit of moving the
school across Hove is about what a quarter of its gap to the top of the
city would buy --- and a single Attainment 8 point is worth roughly
`r sprintf("%.0f", (hp_one_pt - 1) * 100)`% more demand, or about
`r sprintf("%.0f", hp_m(2028, "valley") * (hp_one_pt - 1))` children.

**The answer, then, is: the site helps, and the school's standing
matters more.** The move is a one-off gain of a fixed size. Attractiveness
has four times the headroom and compounds.

And there is a loop here that the model does not close, which is the
most interesting thing in this section. The move itself makes Hove
Park's intake less disadvantaged ---
`r sprintf("%.0f%%", 100 * hp_m(2028, "valley_dep"))` to
`r sprintf("%.0f%%", 100 * hp_m(2028, "nevill_dep"))` in 2028 --- purely
because Nevill Road draws different streets. A less disadvantaged intake
raises the school's raw Attainment 8, because that is largely what raw
Attainment 8 measures (@sec-lever). A higher headline score raises
demand, because that is what families read (@sec-choice). Which raises
the intake again.

That is the Longhill loop (@sec-exec) running forwards instead of
backwards, and the consolidation may start it. Nothing in this model
predicts it: $W_j$ is fixed in every run above, so the figures here are
the move without the loop. What they do establish is where to look, and
the September 2028 and 2029 factsheets will show whether it happens.
**This is a prediction worth recording now**, because a natural
experiment is only useful if the prediction is written down before the
result arrives.

## What if families read a different number? {#sec-value-added-choice}

```{r va-choice-prep}
#| include: false
vac <- bh_data("value_added_choice.rds")
va_city <- function(y, b, col) vac$city[[col]][vac$city$year == y & vac$city$basis == b]
va_sch <- function(y, s, col) {
  d <- vac$schools[vac$schools$year == y, ]
  d[[col]][d$short == s]
}
va_d <- vac$detail %>%
...
va_top <- va_d %>% slice(1)
va_bot <- va_d %>% slice(n())
va26 <- vac$schools %>% filter(year == 2026) %>% arrange(desc(change))
```

Everything above turns on a single fact: the number families read is the
headline score, and the headline score mostly describes the intake
(@sec-choice, @sec-lever). That is a loop, and the council holds one end
of it, because **the council writes the admissions guide** (@sec-exec).
It cannot make families choose differently. It can stop pointing them at
the wrong number, and publish a contextualised one beside it.

So it is worth asking what the city would look like if they read it. The
model can answer, because attractiveness in it is calibrated to what
families ask for, and that can be rebuilt on a different measure. This
run puts the four-year value added score on the Attainment 8 scale and
applies the response families currently show to Attainment 8 — about
`r sprintf("%.0f%%", 100 * (exp(vac$slope) - 1))` more demand per place
per point. Everything else is untouched: the same places, the same
catchments, the same rules, the same journeys. Only the number families
are taken to be reading changes.

It is a large change. `r va_top$short`, whose value added is the city's
strongest relative to its headline score, becomes
`r sprintf("%.1f", va_top$mult)` times as attractive;
`r va_bot$short`, whose value added is the weakest, falls to
`r sprintf("%.1f", va_bot$mult)` times.

```{r va-choice-table}
#| tbl-cap: "Year 7 intakes in 2026 with families reading each measure. Same cohort, same places, same rules."
va26 %>%
  transmute(School = short,
            `Places` = fmt_n(pan),
            `Reading the headline score` = fmt_n(round(att8_intake)),
            `Reading value added` = fmt_n(round(va_intake)),
            Change = sprintf("%+.0f", change),
            `How full it ends up` = sprintf("%.0f%% → %.0f%%",
                                            100 * att8_fill, 100 * va_fill)) %>%
  knitr::kable(align = "lrrrrr")
```

**Brighton Aldridge is the headline.** It goes from
`r fmt_n(round(va_sch(2026, "Brighton Aldridge", "att8_intake")))` children to
`r fmt_n(round(va_sch(2026, "Brighton Aldridge", "va_intake")))`, from half
empty to `r sprintf("%.0f%%", 100 * va_sch(2026, "Brighton Aldridge", "va_fill"))`
full, without a boundary moving or a place being cut. Portslade Aldridge,
Longhill and Hove Park all fill further. The schools that lose are the
ones whose headline scores flatter them: Patcham, whose four-year value
added is the weakest in the city, and Cardinal Newman, whose intake by
2035 falls by
`r fmt_n(abs(round(va_sch(2035, "Cardinal Newman", "change"))))`.

Now the caveats, because they matter as much as the finding.

**It does not create a single place or remove one.** The cohort is what
it is, so the city still has
`r fmt_n(round(va_city(2026, "Value added", "empty")))` empty Year 7
places either way. What changes is *where* they are: on the headline
score the empty places sit at the schools whose viability is in question,
and on value added they sit at schools with room to absorb it. That is
the whole of the effect, and it is not nothing — it is the difference
between a school that looks like it is failing and one that looks full.

**The children sit more evenly across the places the city has — for
now.** There is a single number for this. Ask how many children would
have to change school for every school to be equally full, given the
places that exist. It is zero in a city where every school is at the
same percentage, and it rises with the unevenness however that
unevenness arises.

```{r va-choice-even}
#| tbl-cap: "How evenly the city's children sit across its places, reading each measure. The first column is the number who would have to change school for every school to be equally full."
vac$even %>%
  transmute(`Entry year` = year,
            `Reading the headline score` = fmt_n(round(move_att8)),
            `Reading value added` = fmt_n(round(move_va)),
            `Spread of how full, now` = sprintf("%.0f%% to %.0f%%",
                                                100 * fill_lo_att8, 100 * fill_hi_att8),
            `Spread of how full, then` = sprintf("%.0f%% to %.0f%%",
                                                 100 * fill_lo_va, 100 * fill_hi_va),
            `Empty places in the three emptiest schools` =
              sprintf("%.0f%% → %.0f%%", 100 * top3_att8, 100 * top3_va)) %>%
  knitr::kable(align = "lrrrrr")
```

In 2026 it falls from
`r fmt_n(round(vac$even$move_att8[vac$even$year == 2026]))` children to
`r fmt_n(round(vac$even$move_va[vac$even$year == 2026]))`, a reduction of
about
`r sprintf("%.0f%%", 100 * (1 - vac$even$move_va[vac$even$year == 2026] / vac$even$move_att8[vac$even$year == 2026]))`,
and the fullest-to-emptiest range narrows from
`r sprintf("%.0f%%", 100 * vac$even$fill_lo_att8[vac$even$year == 2026])`
to `r sprintf("%.0f%%", 100 * vac$even$fill_lo_va[vac$even$year == 2026])`
at the bottom. The sharpest number is the last column: today
**`r sprintf("%.0f%%", 100 * vac$even$top3_att8[vac$even$year == 2026])`
of the city's empty Year 7 places sit in just three schools**, and on
value added that falls to
`r sprintf("%.0f%%", 100 * vac$even$top3_va[vac$even$year == 2026])`.
A surplus spread across ten schools is a budgeting problem; a surplus
piled into three is a viability problem, and it is the second the city
actually has.

But read the later years before making too much of it. By
`r vac$even$year[2]` the measure is
`r fmt_n(round(vac$even$move_att8[2]))` either way: the unevenness does
not shrink, it **moves** — out of Brighton Aldridge and Longhill and
into Cardinal Newman and Patcham, which fall well below their numbers in
turn. That is the honest limit of this lever. Changing what families
read can redistribute children across the places that exist; it cannot
conjure children into a city that has
`r fmt_n(round(vac$city$empty[vac$city$year == 2035 & vac$city$basis == "Value added"]))`
more places than children by 2035. Only the places themselves can do
that, which is section 10's argument and not this one's.

**Journeys get shorter, and most for the children who travel furthest.**
Mean journey falls from
`r sprintf("%.1f", va_city(2026, "Attainment 8", "mean_min"))` to
`r sprintf("%.1f", va_city(2026, "Value added", "mean_min"))` minutes, and
the extra distance the most deprived children travel falls from
`r sprintf("%.1f", va_city(2026, "Attainment 8", "dep_gap"))` to
`r sprintf("%.1f", va_city(2026, "Value added", "dep_gap"))` minutes each
way. Children fill the schools near them.

**Segregation rises slightly**, from
`r sprintf("%.3f", va_city(2026, "Attainment 8", "gorard"))` to
`r sprintf("%.3f", va_city(2026, "Value added", "gorard"))` on the index
used throughout section 8. The schools that fill are the more
disadvantaged ones, so their intakes grow. On the evidence in
@sec-concentration-evidence that is not obviously a cost to the children
concerned, but it is a real movement in the measure the council has said
it cares about, and it should not be hidden.

::: {.callout-warning}
## What this is not

It is not a forecast. It assumes families would respond to a
contextualised measure exactly as strongly as they now respond to the
headline one, which is an assumption about publicity and persuasion, not
a fitted quantity. And value added for a single school is a noisy
measure --- Patcham's fall here rests on one school's four-year figure.
The honest claim is narrower than the table looks: **the number families
are pointed at is doing a great deal of work in this system, and it is
the cheapest thing in this document to change.**
:::

You can run this yourself. The simulator has a
[control for what families are choosing on](https://adamdennett-bh-school-system.share.connect.posit.cloud/),
with the scenario "Families choose on value added", and the per-school
attractiveness sliders work on top of it.
...
the council; an academy trust in that position answers to the Department for
Education (@sec-deficit-consequences). The
...
## What being in deficit means, and what the council can do {#sec-deficit-consequences}

@sec-finance-risk says how large each school's gap becomes and roughly
when. This section is about what follows once a school is in it: the
rules that apply, who ends up holding the debt if the school changes
status or closes, what schools do in practice to get out, and what that
implies for a council whose problem is a cohort that is not coming back.

```{r deficit-prep}
#| include: false
who <- fex %>%
  select(urn, name, type, reserve, faith) %>%
  left_join(risk_end %>% filter(scenario == SHED_MID) %>%
              select(urn, gap, gap_pct, years_left), by = "urn") %>%
  mutate(maintained = type != "Academy",
         answers_to = if_else(maintained, "The council (licensed deficit, recovery plan)",
                              "The Department for Education, through the trust"))
n_maint <- sum(who$maintained)
overdrawn <- who %>% filter(reserve <= 0)
cop_d <- bh_data("council_options.rds")
co_d <- function(id, col, y = 2030)
  cop_d$runs[[col]][cop_d$runs$id == id & cop_d$runs$year == y]
lump_now <- unique(fund$lump)
```

### Who answers for which school

```{r tbl-deficit-who}
#| tbl-cap: "The ten schools by who oversees a deficit, with today's reserve and the projected annual gap in 2035 on the middle assumption. Ranked by the 2035 gap."
who %>%
  arrange(gap) %>%
  transmute(School = paste0(name, if_else(faith, " †", "")),
            Status = if_else(maintained, "Maintained", "Academy"),
            `A deficit is overseen by` = answers_to,
            `Reserve now` = gbp(reserve),
            `Gap in 2035` = gbp(gap),
            `Years of reserve` = case_when(
              reserve <= 0 ~ "already overdrawn",
...
**`r n_maint` of the ten are maintained by the council**, including
Cardinal Newman as a voluntary aided school, and **all
`r nrow(overdrawn)` of the schools already overdrawn are among them**:
`r knitr::combine_words(sort(short_sch(overdrawn$name)))`. Whatever the
academies do, most of the city's secondary deficit is the council's to
manage.

### The rules for a maintained school in deficit

The Department for Education's statutory guidance on local schemes for
financing schools sets the framework, and each council writes its own
scheme within it
([DfE, *Schemes for financing local authority maintained schools 2026 to 2027*](https://www.gov.uk/government/publications/schemes-for-financing-schools/schemes-for-financing-local-authority-maintained-schools)).
Three provisions matter here.

- **A recovery plan is compulsory once the deficit passes 5%.** A school
  must submit one when its revenue deficit is above 5% at 31 March.
- **A deficit can be licensed, not forgiven.** The council can agree a
  licensed deficit backed by the collective balances it holds for all
  its schools, repaid over a set period, but the guidance is explicit
  that the council "cannot write off the deficit balance of any school".
- **The council can take control.** It can issue a notice of concern
  that restricts a school's financial freedom and, in the last resort,
  suspend the school's right to a delegated budget.

In Brighton & Hove this is not hypothetical. Local reporting in
[December 2024](https://www.brightonandhovenews.org/2024/12/10/school-leaders-speak-out-as-some-rack-up-six-figure-deficits/)
put 31 schools on course to overspend that year, with 13 not on track to
balance within five years, and in
[October 2025](https://www.brightonandhovenews.org/2025/10/21/schools-funding-cut-by-500k-as-pupil-numbers-keep-falling/)
reported licensed deficits of more than £8 million across 29 schools,
with Cardinal Newman and Hove Park each forecast to end the year more
than £1.7 million in deficit. The council has already cut primary
admission numbers and closed three primary schools. The secondary
sector is where that process has not yet started.

### Where a deficit ends up if the school changes

A deficit does not disappear when a school stops being maintained. The
Department's guidance on academy conversion is specific
([DfE, *Treatment of surplus and deficit balances when maintained schools become academies*, January 2024](https://www.gov.uk/government/publications/academy-conversion-surplus-and-deficit-balance-transfer-process)):

- **A school that chooses to convert takes its deficit with it.** The
  Department reimburses the council and recovers the money from the new
  academy's grant. The council may agree to absorb some or all of it if
  the deficit would otherwise stop the conversion.
- **A school that becomes a sponsored academy leaves its deficit
  behind.** Where a school is eligible for intervention, or is judged
  not strong enough to convert without a sponsor, "the deficit remains
  with the LA, to be funded from its core budget". It cannot be charged
  to the schools budget funded by the Dedicated Schools Grant, except
  from a contingency the schools forum has agreed to hold back for the
  purpose.

A school that closes is governed by the council's own scheme, and
schemes commonly provide that the closing school's balance, surplus or
deficit, reverts to the council. The practical point is the same either
way: **the two schools with accumulated deficits over a million pounds
are both maintained**, and the routes by which either might stop being
maintained leave much or all of that debt with the council rather than
with anyone else.

### What schools actually do to get out

The best evidence on what schools cut is Ofsted's survey of headteachers
([*Making the cut*, February 2020](https://www.gov.uk/government/publications/making-the-cut-how-schools-respond-when-they-are-under-financial-pressure/making-the-cut-how-schools-respond-when-they-are-under-financial-pressure)).
It predates the current funding round and rests on what heads reported,
but it is consistent and specific, and secondary schools reported the
deeper cuts.

```{r tbl-deficit-cuts}
#| tbl-cap: "What secondary headteachers reported doing under financial pressure, and what they reported it did. Ofsted, Making the cut (2020), survey of 103 secondary heads."
tibble::tribble(
  ~`What they did`, ~Share, ~`What they reported it did`, ~`Share `,
  "Reduced teaching assistants", "82%", "Less individual support for pupils", "91%",
  "Reduced teachers", "81%", "Less enrichment", "83%",
  "Reduced non-classroom staff", "77%", "Worse pupil well-being", "80%",
  "Raised class sizes", "91%", "Narrower curriculum", "67%",
  "Cut learning resources", "82%", "Worse behaviour", "57%",
  "Cut professional development", "68%", "Lower quality of teaching", "55%") %>%
  knitr::kable(align = "lrlr")
```

The curriculum losses fell on design and technology, languages and
music, and heads singled out pupils receiving SEN support without an
education, health and care plan as the group losing most. Two further
points from the same report are worth carrying. Nationally, results did
not fall over the decade of real-terms cuts it covered, so the damage is
real but does not show up where it is usually looked for. And the cuts
are mostly to people, which fits what the Brighton & Hove accounts show:
staff costs take between
`r sprintf("%.0f%%", 100 * min(fex$staff_pct))` and
`r sprintf("%.0f%%", 100 * max(fex$staff_pct))`
of the ten schools' income. When @sec-finance-risk assumes a school sheds
three quarters of the funding it loses, it is assuming cuts of this kind.

### What that implies for the city

Advice written for individual schools
([Access Group, 2026](https://www.theaccessgroup.com/en-gb/blog/edu-schools-to-go-into-financial-deficit-how-to-get-out/);
[The Headteacher, 2025](https://www.theheadteacher.com/school-procurement/revenue-generation/why-do-budget-deficits-happen-and-how-do-you-tackle-them))
runs from quick savings --- benchmarking, procurement, energy, supply
cover --- through aligning staffing to the curriculum, to what one of
them calls strategic shifts: federation, rationalising buildings, and
changing admission numbers. For Brighton & Hove the order of those
matters more than any one of them.

1. **A recovery plan cannot close a gap caused by a cohort that is not
   coming back.** Recovery plans assume the pressure is temporary. The
   fall in the Year 7 cohort is already in the primary schools
   (@sec-demography). A school that balances its books by cutting to its
   smaller roll will do so by the cuts in the table above, and will do it
   again the following year.
2. **Taking places out has to be done across the system.** The saving
   from removing places comes from schools whose rolls hold up. Cutting
   places only at the community schools does the opposite: children move
   to the academies and faith schools, and in the model the community
   schools' combined position in 2035 goes from
   `r gbp(co_d("today", "comm_gap", 2035))` to
   `r gbp(co_d("shrink", "comm_gap", 2035))`. Shared across every school
   it improves to `r gbp(co_d("shrink_all", "comm_gap", 2035))`
   (@sec-weighing). That is the case for asking the academies and the
   faith schools to take fewer places too: the council cannot solve this
   with the schools it runs alone.
3. **Federation and amalgamation are different savings.** Two federated
   schools keep their own budgets and their own lump sums and share a
   head, a senior team and back-office costs. Two schools amalgamated
   into one receive one lump sum rather than two --- currently
   `r gbp(lump_now)` a school --- so an amalgamation has to save more
   than that to come out ahead.
4. **Closure moves the problem as well as ending it.** In the model,
   closing Longhill leaves `r co_d("close_lh", "comm_in_deficit")`
   community schools in deficit in 2030 against
   `r co_d("today", "comm_in_deficit")` with it open, because its
   children fill places elsewhere. But its balance comes back to the
   council, its former pupils' journeys lengthen, and the segregation
   and access costs are set out in @sec-weighing.
5. **The timing is set by the reserves.** Hove Park and Cardinal Newman
   are already past the point a recovery plan is required. Longhill's
   reserve covers its projected gap for about
   `r sprintf("%.1f", rk("Longhill")$years_left)` years and Blatchington
   Mill's for about `r sprintf("%.1f", rk("Blatchington")$years_left)`.
   Few of the options in this document could take effect that quickly,
   so the council will be managing licensed deficits while it decides, and should plan the decision
   around when each school's reserve runs out rather than around the
   admissions calendar alone.

::: {.callout-note appearance="simple"}
## What this section adds, and what it does not

**It is a reading of the rules and the evidence, not a legal opinion.**
The council's own scheme for financing schools governs the detail ---
repayment periods, the size of deficit it will license, what happens on
closure --- and should be checked for each of these points.

**Nothing here changes the model.** The projected gaps are the ones in
@sec-finance-risk. What the rules add is who holds them, and what the
evidence adds is what closing them costs pupils.
:::
...
## Every stage of this model, opened up

[The model, stage by stage](model-guide.html) is a companion to this
section. It takes the equation apart one term at a time --- what each
term is, where its numbers come from, how it is computed, a worked
example, the checks that can be run against it, and what it assumes.
The terms are $O_i$, $W_j$, $c_{ij}$, $\beta$, $\gamma_h$, $C_j$ and
$A_i$, followed by the admission-number ceiling and the re-offering of
refused children. Anyone wanting to check a number in this document
rather than take it on trust should start there.
:::
...
::: {.callout-important appearance="simple"}
## The intake figures here remain withdrawn

The catchment figures above measure the neighbourhoods each catchment
contains, and stand. The intake figures below come from this section's
allocation model and score intakes by the neighbourhoods their children
come from, which does not match the schools' published shares of
disadvantaged pupils (@sec-weighing). The full model has been
recalibrated to those shares; this allocation model has not, so these
intake figures remain withdrawn.
:::
...
The first version of this subsection measured intakes by the
neighbourhoods their children come from, which did not match the
schools' published shares of disadvantaged pupils (@sec-weighing). It
found priority 6 made intakes noticeably more segregated. On the
recalibrated measure the effect is much smaller, and the mechanisms the
first version described are not repeated here.
:::
...
places, then priority 6 up to its share, then the catchment.
...
p6s$sweep %>%
  ...
  labs(x = "Priority-6 share of each community school's places", y = "Gorard's segregation index",
       title = "More places for single-school catchments barely changes how mixed intakes are",
...
**Priority 6 makes almost no difference to how mixed the intakes are.** In
...
children at `r p6_sat`%, `r fmt_n(p6_n(40))` at 40%. Between none and
15% the index moves by
`r sprintf("%+.0f", 100 * (p6_g(15) / p6_g(0) - 1))`%, and by 2030 by
`r sprintf("%+.0f", 100 * (p6_g(15, 2030) / p6_g(0, 2030) - 1))`% --- well
inside what the model's assumptions could move.

```{r tbl-p6-schools}
#| tbl-cap: "Each school's modelled intake, with priority 6 off and at 15% of places, under the council's priorities in 2026: its share of disadvantaged pupils (calibrated to the published shares), and its contribution to Gorard's index (half the absolute gap between its share of the city's disadvantaged pupils and its share of all pupils). Ordered by how much the school adds to the index."
p6s$schools %>%
...
**Who uses it depends on the catchment.** From Longhill's catchment the
priority-6 places go mostly to the less disadvantaged families:
`r fmt_pct(100 * p6_w("Longhill", "dep_p6"), 0)` of them to disadvantaged
children, where the catchment as a whole is
`r fmt_pct(100 * p6_w("Longhill", "dep_all"), 0)` disadvantaged. From
Portslade's it is the other way round,
`r fmt_pct(100 * p6_w("PACA", "dep_p6"), 0)` against
`r fmt_pct(100 * p6_w("PACA", "dep_all"), 0)`; from Brighton Aldridge's,
`r fmt_pct(100 * p6_w("BACA", "dep_p6"), 0)` against
`r fmt_pct(100 * p6_w("BACA", "dep_all"), 0)`. It does not move the
city's index because the places are few and go to schools on both sides
of the city's share.
...
went to. Each school's pull on disadvantaged children is fitted to its
2026 Year 7 offers, or for the academies and faith schools its published
share, and held fixed, which is an assumption about how the children who
use priority 6 differ from their neighbours. And
segregation is one objective among several. Priority 6 widens access for
the children who use it --- the `r fmt_n(p6_n(5))` or so a year at 5% ---
and on this evidence it does so at almost no cost to how mixed the
intakes are. The simulator shows the same comparison live in its
Fairness tab for any setting.
:::
...
- **Moving with redrawn catchments gains children**: `r fmt_n(m5n("E", 2030))` in 2030 against `r fmt_n(m5n("B", 2030))` staying put. The redraw wins back what the move lost and a little more.
...
**Its effect on how mixed intakes are is small.** Counting disadvantaged
pupils as calibrated in @sec-weighing, Gorard's index goes from
`r sprintf("%.3f", cm_v("today", "gorard"))` to
`r sprintf("%.3f", cm_v("comart", "gorard"))` in 2026, and from
`r sprintf("%.3f", cm_v("today", "gorard", 2030))` to
`r sprintf("%.3f", cm_v("comart", "gorard", 2030))` in 2030. A school
that recruits locally takes its neighbourhood with it:
`r fmt_pct(100 * cm_s("comart", "CoMArt", "dep_share"), 0)` of CoMArt's
intake is disadvantaged, against `r fmt_pct(100 * cm_city_dep, 0)` across
the city. But Longhill and Brighton Aldridge, the two most disadvantaged
intakes, are already well above that, so a third school in the east
between them does little to the city's index. Brighton Aldridge goes from
`r fmt_pct(100 * cm_s("today", "Brighton Aldridge Community Academy", "dep_share"), 0)`
to `r fmt_pct(100 * cm_s("comart", "Brighton Aldridge Community Academy", "dep_share"), 0)`,
Longhill from
`r fmt_pct(100 * cm_s("today", "Longhill High School", "dep_share"), 0)`
to `r fmt_pct(100 * cm_s("comart", "Longhill High School", "dep_share"), 0)`.
What CoMArt changes is access: it shortens deprived children's journeys
without, on this evidence, concentrating them much more than today's
system already does.

```{r tbl-comart-schools}
#| tbl-cap: "Each school's modelled intake and its share of disadvantaged pupils (calibrated to the published shares), today and with CoMArt open at 150 (Brighton Aldridge and Longhill at 150), council priorities, 2026."
cms$schools %>%
...
`r sprintf("%.3f", cm_sens("As Longhill", "gorard"))`, against
`r sprintf("%.3f", cm_v("today", "gorard"))` today. At the city median it
fills and the index is
`r sprintf("%.3f", cm_sens("The city median", "gorard"))`. Both the size
and the direction of CoMArt's effect on segregation depend on this
assumption; its effect on journeys does not.
...
admissions guide, and it tells parents to look at each school's
"National Curriculum test results, public exam results"
([2027 to 2028](https://www.brighton-hove.gov.uk/schools-and-learning/school-policies-reports-strategies-and-other-documents/secondary-school-admissions-guide-2027-2028)). Section 5 shows families track the headline score at
...
2. **Which neighbourhoods did the priority-6 places go to, and what did
   ...
   (@sec-fr-p6) In the full model it makes almost no difference to how
   mixed intakes are, and who uses it varies by catchment. The council holds the offers and
   the home postcodes to test both.
...
| 5 | Children allocated a school they did not name, by school, year and home catchment | Test the council's stated "nearest school with places" practice, and complete each school's intake: the published offer counts are counted by preference rank, so a child placed at a school they did not name does not appear in them |
| 6 | Journey actually made, or at least distance, per allocation | Separate potential accessibility from realised accessibility |
| 7 | Offers by admission criterion for every school, every year | The admissions guide gives the priority breakdown only for the schools that rationed places; the rest would show how the tiered ceiling behaves where it barely binds |
| 8 | Offers to children living outside the city, by school | The model contains Brighton & Hove children only. Schools that admit across a wider area --- the faith schools above all --- fill some places the model cannot see |
| 9 | The catchment preference table for each round separately, not pooled | Let each school's attractiveness be fitted year by year, so a change such as a new Ofsted judgement shows up when it happens rather than being averaged away |
| 10 | In each paired catchment, how many applicants named one school of the pair, both, or neither | Fit the share who would take only one of the pair each year, rather than holding it fixed |

```{r data-asks-fp}
#| include: false
fp_all <- bh_data("factsheet_panel.rds")$factsheets
fp_of <- function(p, y, col) {
  v <- fp_all[[col]][grepl(p, fp_all$name) & fp_all$year == y]
  stopifnot(length(v) == 1)
  v
}
```

The last four follow from where the model's remaining error sits. Its
intakes for September 2026 match the council's offers closely at most
schools and miss at two: it gives Brighton Aldridge too few children and
Hove Park too many
([model guide](https://adamdennett.github.io/bh-school-system/model-guide.html#sec-end)).
The published factsheets show why both are hard for a model fitted to a
pooled table. Brighton Aldridge's first preferences rose from
`r fp_of("Brighton Aldridge", 2025, "pref1")` to
`r fp_of("Brighton Aldridge", 2026, "pref1")` in the first round after
its April 2025 Ofsted judgement, and Hove Park's second preferences fell
from `r fp_of("Hove Park", 2025, "pref2")` to
`r fp_of("Hove Park", 2026, "pref2")` in the same round. A model whose
attractiveness is an average of earlier rounds cannot follow a change
that happens in one. Releases 9 and 10 would let it, and releases 5, 7
and 8 would let the model be checked against every child placed rather
than only those offered a school they named.
...
### What one small table already settled {#sec-eef-ward}
...
The case for these releases is not hypothetical, because one of them has
in effect already happened. Appendix 1 of the council's Educational
Equity Framework, prepared for the People Overview & Scrutiny Committee,
counts Key Stage 2 and Key Stage 4 pupils by home ward and by middle
super output area, taken from the January 2025 school census. It was
published to describe attainment, not pupil numbers, and the counts sit
in it as a by-product.

Those counts are the first external check this model's **origins** have
ever had. Every other test in this document tests where the model sends
children; nothing tested how many it starts with in each neighbourhood,
and that is built from ONS ward estimates and a single state-sector
share for the whole city. Set the council's Key Stage 2 count against
modelled intakes into city schools, ward by ward, and the geography
holds: the two correlate at
`r sprintf("%.2f", cor(wv$compare$modelled, wv$compare$ks2))`. The flat
share does not. It implies a share of each ward's children in city
schools running from about `r sprintf("%.2f", wv_lo$share)` in
`r wv_lo$ward` to about `r sprintf("%.2f", wv_hi$share)` in
`r wv_hi$ward`, against the
`r sprintf("%.2f", wv$flat_share)` the model applies everywhere.
Rebuilding the model's origins on per-ward shares moves its intakes
closer to the offers the city's schools actually made in September 2026,
on every measure of fit, although the offers play no part in deriving
the shares. The
[model guide](https://adamdennett.github.io/bh-school-system/model-guide.html#sec-oi-ward-check)
sets out the comparison in full; the results in this document still use
the flat share.

Two things follow. The first is a correction the council could make to
its own place planning: how many of an area's children take a city state
school place varies far more across the city than a single rate allows,
and the variation is large enough to change which schools look full.
The second is the general point. A table published for another purpose
entirely, containing nothing more sensitive than counts of pupils by
ward, answered a question that had been stated as unanswerable. The ten
releases above are of the same kind.
...
# Weighing the priorities {#sec-weighing}

The council is trying to keep a financially viable system of secondary
schools, and it has more than one thing it wants from it. This section
takes three priorities in turn --- keeping Longhill open; better access
for disadvantaged children, and less segregation; and, a parent's
priority more than a council one, children at a school in their own
catchment, near home, and not displaced --- and then weighs them
against each other.

It is a recommendation from a model, not from the council's records, and
it carries every limit stated in section 7. It is also written knowing
what the council can and cannot do. It sets the admission numbers of the
six community schools, the catchment map, Longhill's site and the
oversubscription priorities. It cannot set the numbers at Cardinal
Newman, King's, Brighton Aldridge or Portslade Aldridge --- but those
schools take a third of the city's children, and where the evidence says
the rest of the system would be better for their taking fewer, this
section says so.

::: {.callout-important appearance="default"}
## How segregation is measured here, and a check the first version failed

```{r weigh-validate}
#| tbl-cap: "Each school's share of disadvantaged pupils: as the first version of the model measured it, by the neighbourhoods its children come from; as the Department for Education publishes it for the whole school (the mean of 2022-23 to 2024-25, every cohort admitted before the 2024 map and the new priorities); the share of each community school's September 2026 Year 7 offers that went to children eligible for free school meals (council allocation factsheet); and the model's Year 7 share in 2026, which reproduces the offers for the community schools and the published share for the rest. The last three columns are each school's pull on disadvantaged children fitted three ways: to the Year 7 offers (the model's measure), to the published shares on the pre-2024 map and rules, and to the published shares on today's map. Ordered by the published share."
val <- bh_data("council_options.rds")$dis_fit %>%
  mutate(short = vapply(name, short_sch, character(1)),
         nb = 100 * neighbourhood, pub = 100 * published, cal = 100 * modelled,
         nb_side = ifelse(nb >= weighted.mean(nb, intake), "above", "below"),
         pub_side = ifelse(pub >= weighted.mean(pub, intake), "above", "below"))
val_r <- cor(val$nb, val$pub)
val_r_nobaca <- with(val %>% filter(short != "Brighton Aldridge"), cor(nb, pub))
val_same <- sum(val$nb_side == val$pub_side)
val_of <- function(s, col) val[[col]][val$short == s]
val %>%
  arrange(desc(pub)) %>%
  transmute(School = short,
            `First version: by neighbourhood` = sprintf("%s (%s)", fmt_pct(nb, 0), nb_side),
            `Published: disadvantaged pupils` = sprintf("%s (%s)", fmt_pct(pub, 0), pub_side),
            `Year 7 offers, 2026: free school meals` = ifelse(is.na(year7_fsm), "---", fmt_pct(100 * year7_fsm, 0)),
            `Model, Year 7 2026` = fmt_pct(cal, 0),
            `Pull: Year 7 offers` = sprintf("%.2f", lambda),
            `Pull: before the change` = sprintf("%.2f", lambda_pre),
            `Pull: today's map, whole school` = sprintf("%.2f", lambda_today_map)) %>%
  knitr::kable(align = "lrrrrrrr")
```

The first version of this section measured how mixed a school's intake
is by the **neighbourhoods** its children come from. Checked against the
share of **disadvantaged pupils each school actually has**, as the
Department for Education publishes it, that measure put the schools in
the wrong order. The two correlate at `r sprintf("%.2f", val_r)`, and
only because of Brighton Aldridge: without it the correlation is
`r sprintf("%.2f", val_r_nobaca)`, and only `r val_same` of the
`r nrow(val)` schools are on the same side of the city average on both.
Longhill and Hove Park, which the neighbourhood measure put below the
average, have among the highest disadvantaged shares in the city ---
`r fmt_pct(val_of("Longhill", "pub"), 0)` and
`r fmt_pct(val_of("Hove Park", "pub"), 0)` --- and Varndean, which it put
well above, has about the average.

The gap is families sorting *within* neighbourhoods: in a mixed
neighbourhood, the disadvantaged families and the better-off ones do not
choose the same schools, and a neighbourhood measure cannot see it. That
sorting is what segregation is made of, so **the model has been
recalibrated to it**. Each neighbourhood's disadvantaged children are
placed across the schools its children attend, weighted by each
school's pull on disadvantaged children, and those pulls are fitted to
what each school actually admits.

**The published shares lag the policy.** They average Years 7 to 11
over 2022-23 to 2024-25, and every one of those cohorts was admitted
before September 2025, under the pre-2024 map with no free school meals
priority and no priority 6. Fitted to them on today's map, the pulls
absorb the policy change itself. Fitted on the old map and rules, they
still miss what the new rules do. So for the six community schools the
target is the first intake admitted under today's map and priorities:
the share of each school's September 2026 offers that went to children
eligible for free school meals, which the council publishes in its
allocation factsheet. The four academies and faith schools do not
publish it and their arrangements did not change in 2025, so they keep
their published whole-school shares. Free school meal eligibility is a
narrower count than the Pupil Premium measure, so if anything this
understates the community schools' disadvantaged intakes. The pulls are
then held fixed while maps, admission numbers and schools change. Every segregation
figure in this section, in the priority-6 subsection (@sec-fr-p6) and in
CoMArt's (@sec-comart) is on that basis. The intake figures in section 8
come from a separate allocation model the recalibration does not cover,
and remain withdrawn.

**Several results change.** On this measure returning Whitehawk to
Longhill's catchment raises segregation --- the box under Priority 2
sets out why earlier versions said otherwise, and how far the finding
holds. Priority 6 makes no measurable difference. Closing Longhill,
removing the oversubscription priorities and trimming Cardinal Newman
and King's lower segregation; the power diagram, Elm Grove and the
packages raise it. Several options differ by only a few thousandths on
the index, which is inside what a change of measure has moved, so only
the larger differences should be read as findings. The journey, displacement, catchment and
Longhill results do not rest on this measure and are unchanged.
:::
...
co_lab <- function(id) cop$options[[id]]$label
# Longhill's modelled annual balance moves in a straight line with its
# intake (section 6's steady-state roll), so the intake at which it
# breaks even can be read off the runs.
be_fit <- lm(lh_gap ~ lh_intake, data = cop$runs %>% filter(is.finite(lh_gap)))
lh_breakeven <- -coef(be_fit)[[1]] / coef(be_fit)[[2]]
lh_per_child <- coef(be_fit)[[2]]
gbp_m <- function(x) sprintf("%s£%.1fm", ifelse(x < 0, "−", ""), abs(x) / 1e6)
d_or <- function(x, f) ifelse(is.na(x), "—", f(x))
```

## How the options were scored

Every option below was run on the simulator's model --- the full model
of section 7, with the council's 2026/27 priorities, the East Sussex
schools children leave for, and refused children re-offered by second
preference --- in 2026, 2030 and 2035. Each is scored on all three
priorities at once: Longhill's intake and finances, and the community
schools' finances; Gorard's segregation index and how much further
deprived children travel than everyone else; and how many children are
displaced, how many attend a school in their own catchment, and how long
the longest journeys are. The same options, and any variation on them,
can be run in the simulator.

One new map is in them. The 2024 catchment change moved the
neighbourhoods around CoMArt's old site in Whitehawk --- six zones,
about 60 children a year --- out of Longhill's catchment into Stringer /
Varndean's, while moving others further out the other way. **"Whitehawk
back to Longhill"** is today's map with only those neighbourhoods
returned.

```{r tbl-weigh-2030}
#| tbl-cap: "The options in 2030, under the council's priorities. Community schools' balance is the steady-state annual balance across the six schools the council runs (section 6). Gorard is across all city schools' intakes, counting disadvantaged pupils calibrated to each school's published share (see the box at the head of this section). The deprived-journey gap is the extra minutes children from the three most deprived deciles travel. Displaced children would have taken a place in their own catchment and could not. Where an option cuts a school's admission number and that school is no longer full in 2030, the cut changes nothing that year."
show <- c("today", "lh150", "central", "whitehawk", "whitehawk_central", "elm",
          "lh_wanted", "cn300", "cn270_kings", "academies", "p6_off", "p6_20",
          "equal", "power", "comart", "package_d", "package_e", "package_f", "close_lh")
cop$runs %>%
  filter(year == 2030, id %in% show) %>%
  mutate(id = factor(id, show)) %>% arrange(id) %>%
  transmute(Option = label,
            Longhill = d_or(lh_intake, function(x) fmt_n(x)),
            `Community schools' balance` = gbp_m(comm_gap),
            Gorard = sprintf("%.3f", gorard),
            `Deprived-journey gap` = sprintf("%.1f min", ifelse(abs(dep_gap) < 0.05, 0, dep_gap)),
            `Longest journeys (90th %ile)` = sprintf("%.0f min", p90_min),
            Displaced = fmt_n(displaced),
            `At a catchment school` = fmt_pct(100 * in_catchment, 0)) %>%
  knitr::kable(align = "lrrrrrrr")
```

## Priority 1: keeping Longhill open

**No option tested makes Longhill pay its way.** On section 6's funding
assumptions Longhill breaks even at about
**`r fmt_n(lh_breakeven)` children a year**; each child is worth about
`r paste0("£", fmt_n(lh_per_child))` a year to its balance. Today it
takes `r fmt_n(co("today", "lh_intake", 2026))` in 2026 and
`r fmt_n(co("today", "lh_intake", 2035))` by 2035. The best of the
options below take it to around a hundred for a few years and to the
eighties by 2035. Keeping Longhill open is therefore a decision to run
a smaller school at a cost --- to plan its staffing and curriculum for
about a hundred children a year rather than to wait for 150 --- and the
council should take it as that, openly, rather than as a bet on
recovery.

**Cutting its admission number removes empty places; it does not add
children.** At 150 or 120 places Longhill takes exactly the children it
takes at 210, and its balance is unchanged. The cut is still worth
making --- `r fmt_n(co("today", "comm_empty") - co("lh150", "comm_empty"))`
empty places a year gone from the community schools --- but it is
housekeeping, not rescue.

The levers that do move children, ranked by what they add in 2030:

- **Whitehawk back in Longhill's catchment** adds
  `r fmt_n(co("whitehawk", "lh_intake") - co("today", "lh_intake"))` children
  (`r fmt_n(co("whitehawk", "lh_intake", 2026))` in 2026,
  `r fmt_n(co("whitehawk", "lh_intake", 2035))` in 2035, against
  `r fmt_n(co("today", "lh_intake", 2026))` and
  `r fmt_n(co("today", "lh_intake", 2035))`), and keeps most of that
  gain as the cohort falls. It barely changes journeys, and Priority 3
  shows it lowers displacement. It raises segregation, by more than any
  other option short of moving a school (Priority 2).
- **Moving to Elm Grove with redrawn catchments** adds
  `r fmt_n(co("elm", "lh_intake") - co("today", "lh_intake"))` in 2030 and
  `r fmt_n(co("elm", "lh_intake", 2026) - co("today", "lh_intake", 2026))`
  in 2026, more than any other single change the council controls, and
  the map drawn round the new site puts
  `r cop$elm_moves$whitehawk_to_longhill` of the
  `r cop$elm_moves$whitehawk_zones` Whitehawk neighbourhoods back in
  Longhill's catchment. It needs a new site, which is not costed here,
  and it raises segregation (Priority 2).
- **Making Longhill half as attractive again** adds `r fmt_n(co("lh_wanted", "lh_intake") - co("today", "lh_intake"))`.
  On section 5.4's fit that is about
  `r sprintf("%.1f", log(1.5) / ch_w5_slope)` points of Attainment 8,
  which a school with a falling roll is poorly placed to deliver.
- **Cardinal Newman and King's taking fewer children** --- Cardinal
  Newman at 270 and King's at 150 --- adds
  `r fmt_n(co("cn270_kings", "lh_intake") - co("today", "lh_intake"))` in
  2030 and `r fmt_n(co("cn270_kings", "lh_intake", 2026) - co("today", "lh_intake", 2026))`
  in 2026, and does more for the community schools' finances than any
  other single change the council does not control: their combined
  balance improves by
  `r gbp_m(co("cn270_kings", "comm_gap", 2035) - co("today", "comm_gap", 2035))`
  a year by 2035. Cardinal Newman at 300 alone does about half of that.
- **Trimming the full central schools** --- Stringer to 300, Varndean to
  270, Blatchington Mill to 300 --- adds
  `r fmt_n(co("central", "lh_intake", 2026) - co("today", "lh_intake", 2026))`
  in 2026 and **nothing by 2030**, because by then the falling cohort has
  left those schools with room of their own. It is a lever for the next
  two or three rounds only, and it lengthens journeys for the children it
  moves unless it goes with a redraw that gives them a nearer school.

**Asking Brighton Aldridge and Portslade Aldridge to shrink does nothing
for Longhill.** Neither fills, so a lower number only removes their own
empty places.

## Priority 2: access and segregation

These are two different things, and the options that help one often hurt
the other.

**Access** --- how much further deprived children travel than everyone
else --- is improved most by putting a school where they live. With
Longhill at Elm Grove the gap falls from
`r sprintf("%.1f", co("today", "dep_gap"))` to
`r sprintf("%.1f", max(0, co("elm", "dep_gap")))` minutes; with CoMArt open, to
`r sprintf("%.1f", co("comart", "dep_gap"))`. Nothing else in the table
moves it by more than half a minute.
...
**For Whitehawk the move does what the redraw cannot.** With Longhill at
Elm Grove, Whitehawk's children travel
`r sprintf("%.0f", whe("elm", "minutes"))` minutes on average in 2026
against `r sprintf("%.0f", whe("today", "minutes"))` today, and its
disadvantaged children `r sprintf("%.0f", whe("elm", "minutes_disadvantaged"))`
against `r sprintf("%.0f", whe("today", "minutes_disadvantaged"))`.
Returning Whitehawk to Longhill's catchment with the school at Ovingdean
leaves its disadvantaged children at
`r sprintf("%.0f", whe("whitehawk", "minutes_disadvantaged"))` minutes,
because Ovingdean is no nearer to them than Varndean is.

**Segregation** is lowest where the intakes are mixed, and a school
placed among deprived neighbourhoods recruits from them: Gorard's index
rises from `r sprintf("%.3f", co("today", "gorard"))` to
`r sprintf("%.3f", co("elm", "gorard"))` with Longhill at Elm Grove and to
`r sprintf("%.3f", co("comart", "gorard"))` with CoMArt open (@sec-comart).

**Returning Whitehawk to Longhill's catchment raises segregation.** In
2030 the index is `r sprintf("%.3f", co("today", "gorard"))` today and
`r sprintf("%.3f", co("whitehawk", "gorard"))` with the redraw; in 2026,
`r sprintf("%.3f", co("today", "gorard", 2026))` and
`r sprintf("%.3f", co("whitehawk", "gorard", 2026))`. That is the largest
rise of any option the council can take without moving or opening a
school. Earlier versions of this section found first that the redraw
lowered segregation and then that it barely moved it; the box below sets
out what changed, and how far the finding now holds.

::: {.callout-note appearance="default"}
## Does returning Whitehawk to Longhill change segregation?

```{r wh-prep}
#| include: false
whx <- bh_data("whitehawk_explained.rds")
wh_g <- function(map, y = 2026) whx$gorard$gorard[whx$gorard$map == map & whx$gorard$year == y]
wh_s <- function(school, map, col, y = 2026)
  whx$schools[[col]][whx$schools$short == school & whx$schools$map == map & whx$schools$year == y]
wh_d <- function(school, map, col, y = 2026) {
  v <- whx$dest[[col]][whx$dest$short == school & whx$dest$map == map & whx$dest$year == y]
  if (length(v)) v else 0
}
wh_m <- function(measure, map, col = "gorard", y = 2026)
  whx$measures[[col]][whx$measures$measure == measure & whx$measures$map == map & whx$measures$year == y]
wh_sens <- function(variant, map) whx$sens$gorard[whx$sens$variant == variant & whx$sens$map == map]
wh_city <- with(whx$schools %>% filter(map == "today", year == 2026), sum(dis) / sum(intake))
wh_fit <- function(s, col) { f <- whx$dis_fit; f[[col]][vapply(f$name, short_sch, character(1)) == s] }
wh_chg <- vapply(c("As fitted", "Half the pull", "No pull"),
                 function(v) wh_sens(v, "whitehawk") - wh_sens(v, "today"), numeric(1))
wh_fmt <- function(x) if (abs(x) < 0.0005) "less than a thousandth" else sprintf("%+.3f", x)
```

**Who lives there.** The 2024 catchment change moved
`r whx$area$n_zones` small areas around CoMArt's old site in Whitehawk
out of Longhill's catchment and into Stringer / Varndean's. About
`r fmt_n(whx$area$children)` children a year live in them, and
`r fmt_pct(100 * whx$area$disadvantaged_share, 0)` of them are
disadvantaged --- against `r fmt_pct(100 * whx$area$city_share, 0)` across
the city, `r fmt_pct(100 * whx$area$dsv_share, 0)` in the rest of
Stringer / Varndean's catchment, and
`r fmt_pct(100 * whx$area$longhill_share, 0)` in Longhill's.

**Where their children go.** Today
`r fmt_n(wh_d("Varndean", "today", "children"))` go to Varndean and
`r fmt_n(wh_d("Dorothy Stringer", "today", "children"))` to Stringer.
Returned to Longhill's catchment,
`r fmt_n(wh_d("Longhill", "whitehawk", "children"))` go to Longhill ---
in the model almost all of them disadvantaged ones, for the reason set
out below --- and Cardinal Newman
takes `r fmt_n(wh_d("Cardinal Newman", "whitehawk", "children"))` rather
than `r fmt_n(wh_d("Cardinal Newman", "today", "children"))`. For the
children who go to Longhill the journey is about
`r sprintf("%.0f", wh_d("Longhill", "whitehawk", "minutes"))` minutes,
against `r sprintf("%.0f", wh_d("Varndean", "today", "minutes"))` to
Varndean.

```{r tbl-whitehawk}
#| tbl-cap: "The three schools the Whitehawk redraw moves, 2026, council priorities: each school's share of disadvantaged pupils, and its contribution to Gorard's index (half the gap between its share of the city's disadvantaged pupils and its share of all pupils)."
whx$schools %>%
  filter(year == 2026, short %in% c("Longhill", "Varndean", "Dorothy Stringer")) %>%
  select(short, map, share, contrib) %>%
  tidyr::pivot_wider(names_from = map, values_from = c(share, contrib)) %>%
  transmute(School = short,
            `Disadvantaged, today` = fmt_pct(100 * share_today, 1),
            `With Whitehawk back` = fmt_pct(100 * share_whitehawk, 1),
            `Contribution to the index, today` = sprintf("%.4f", contrib_today),
            `With Whitehawk back ` = sprintf("%.4f", contrib_whitehawk)) %>%
  knitr::kable(align = "lrrrr")
```

**What it does to the intakes.** Across the city
`r fmt_pct(100 * wh_city, 1)` of pupils are disadvantaged. Longhill's
intake, already above that at
`r fmt_pct(100 * wh_s("Longhill", "today", "share"), 1)`, rises to
`r fmt_pct(100 * wh_s("Longhill", "whitehawk", "share"), 1)`, which adds
to the index. Varndean's, also above it today at
`r fmt_pct(100 * wh_s("Varndean", "today", "share"), 1)`, falls to
`r fmt_pct(100 * wh_s("Varndean", "whitehawk", "share"), 1)`, passing
below it. Stringer's goes from
`r fmt_pct(100 * wh_s("Dorothy Stringer", "today", "share"), 1)` to
`r fmt_pct(100 * wh_s("Dorothy Stringer", "whitehawk", "share"), 1)`,
further below it again. All three schools end further from the city's
share than they started, and Gorard's index measures exactly that: it is
`r sprintf("%.3f", wh_g("today"))` today and
`r sprintf("%.3f", wh_g("whitehawk"))` with the redraw in 2026,
`r sprintf("%.3f", wh_g("today", 2030))` and
`r sprintf("%.3f", wh_g("whitehawk", 2030))` in 2030.

**Why earlier versions said otherwise.**

```{r tbl-wh-measures}
#| tbl-cap: "Gorard's index, and Longhill's and Varndean's shares of disadvantaged pupils, today and with Whitehawk returned to Longhill, 2026, council priorities, on each of the four ways this document has counted disadvantaged pupils. The last is the model's measure."
whx$measures %>%
  filter(year == 2026) %>%
  select(measure, map, gorard, longhill_share, varndean_share) %>%
  tidyr::pivot_wider(names_from = map, values_from = c(gorard, longhill_share, varndean_share)) %>%
  transmute(`Disadvantaged pupils counted` = measure,
            `Index, today` = sprintf("%.3f", gorard_today),
            `With Whitehawk back` = sprintf("%.3f", gorard_whitehawk),
            `Longhill` = sprintf("%s → %s", fmt_pct(100 * longhill_share_today, 0), fmt_pct(100 * longhill_share_whitehawk, 0)),
            `Varndean` = sprintf("%s → %s", fmt_pct(100 * varndean_share_today, 0), fmt_pct(100 * varndean_share_whitehawk, 0))) %>%
  knitr::kable(align = "lrrrr")
```

Scored by the neighbourhoods their children come from, Varndean looked
the more deprived school and Longhill the less, so moving Whitehawk
looked like evening them out. The published figures put them the other
way round, and the second version fitted each school's pull on
disadvantaged children to those figures --- but on today's map. The
published figures are whole-school averages for cohorts admitted before
the 2024 map and the new priorities, when Whitehawk was in Longhill's
catchment. Fitted on today's map, Longhill's pull
(`r sprintf("%.2f", wh_fit("Longhill", "lambda_today_map"))`) stood in
for Whitehawk children it no longer admits, and Varndean's
(`r sprintf("%.2f", wh_fit("Varndean", "lambda_today_map"))`) held down
the ones it now does; moving Whitehawk then looked like concentrating
disadvantage at Longhill. Fitted on the map and rules those cohorts were
admitted under, the pulls are
`r sprintf("%.2f", wh_fit("Longhill", "lambda_pre"))` and
`r sprintf("%.2f", wh_fit("Varndean", "lambda_pre"))`, and the redraw
barely moves the index --- but that fit still misses what the new rules
do. Fitted to the first Year 7 offers under today's map and priorities,
the model's measure, Longhill's pull is far stronger,
`r sprintf("%.2f", wh_fit("Longhill", "lambda"))`, because
`r fmt_pct(100 * wh_fit("Longhill", "year7_fsm"), 0)` of its September
2026 offers went to children eligible for free school meals; Varndean's
is `r sprintf("%.2f", wh_fit("Varndean", "lambda"))`. On that measure the
redraw raises the index a little, because the model sends Longhill
almost only the disadvantaged Whitehawk children:
`r fmt_n(wh_d("Longhill", "whitehawk", "dis"))` of
`r fmt_n(wh_d("Longhill", "whitehawk", "children"))`.

**How firm that is.** As fitted, the redraw changes the index by
`r wh_fmt(wh_chg[["As fitted"]])`; at half the fitted pull by
`r wh_fmt(wh_chg[["Half the pull"]])`; with no pull by
`r wh_fmt(wh_chg[["No pull"]])`.
`r if (wh_chg[["As fitted"]] > 0 && wh_chg[["Half the pull"]] > 0) "It rises at the fitted pull and still rises at half of it, and only reverses when the pull is removed altogether --- which is the version that no longer reproduces the council's offers. The direction is as firm as the fitted pull is, and the size of it is not." else "The sign changes with the strength of the pull, so the direction is not firm."`
Longhill's pull rests on a single year of offers at a school that made
fewer than a hundred of them on offer day, and it implies that almost
every Whitehawk family choosing Longhill would be disadvantaged. A
second year of Year 7 offers would test it; the published whole-school
figures will not show the 2024 map in full until every year group has
been admitted under it, in 2029-30.

**What that leaves.** The redraw is a genuine trade. Against the rise in
the index sit a shorter journey for the children who would take a
Longhill place, `r fmt_n(co("whitehawk", "lh_intake") - co("today", "lh_intake"))`
more children a year for Longhill by 2030, and less displacement. For
comparison, the whole pre-2024 map scores
`r sprintf("%.3f", co("pre2024", "gorard", 2026))` in 2026, against
`r sprintf("%.3f", co("today", "gorard", 2026))` today. And a more or
less mixed intake is not the same as
better or worse results for the children in it: most of the difference
between schools' headline scores reflects who they teach rather than what
they add (@sec-school-effects).
:::

**Priority 6 makes almost no difference.** Switched off, the index is
`r sprintf("%.3f", co("p6_off", "gorard"))`; at 20% it is
`r sprintf("%.3f", co("p6_20", "gorard"))`
([the priority-6 subsection of section 8](#sec-fr-p6)). Removing all
oversubscription priorities gives
`r sprintf("%.3f", co("equal", "gorard"))` and displaces
`r fmt_n(co("equal", "displaced", 2026))` children in 2026 against
`r fmt_n(co("today", "displaced", 2026))`.

**The power diagram raises segregation** on this model ---
`r sprintf("%.3f", co("power", "gorard"))` --- and costs Longhill
`r fmt_n(co("today", "lh_intake") - co("power", "lh_intake"))` children.
It is not recommended on this evidence.

## Priority 3: catchments, journeys and displacement

For parents the first thing is a short journey to a school near home and
friends, and the second is not being displaced from it.

**Displacement is small and falling.**
`r fmt_n(co("today", "displaced", 2026))` children in 2026 and
`r fmt_n(co("today", "displaced", 2030))` in 2030 would have taken a
place in their catchment and cannot. Options that trim the full schools
hard raise it --- to `r fmt_n(co("central_hard", "displaced", 2026))` in
2026 with Stringer at 270, Varndean at 240 and Blatchington Mill at 270
--- and so does removing the priorities. Whitehawk back to Longhill
lowers it, to `r fmt_n(co("whitehawk", "displaced", 2026))` in 2026,
because the Whitehawk families who would be refused at a full central
school now have a catchment school with room.

**The share of children at a school in their own catchment is raised
most by the faith schools taking fewer** ---
`r fmt_pct(100 * co("cn270_kings", "in_catchment"), 0)` with Cardinal
Newman at 270 and King's at 150, against
`r fmt_pct(100 * co("today", "in_catchment"), 0)` --- because every
child they do not take goes to a catchment school instead, and usually a
nearer one: the longest journeys fall to
`r sprintf("%.0f", co("cn270_kings", "p90_min"))` minutes from
`r sprintf("%.0f", co("today", "p90_min"))`.

**Closing Longhill is the option parents would pay most for.** It saves
the most money --- the community schools' balance is
`r gbp_m(co("close_lh", "comm_gap", 2026))` in 2026 against
`r gbp_m(co("today", "comm_gap", 2026))`, partly because Longhill's own
deficit leaves the books --- but the longest journeys
lengthen to `r sprintf("%.0f", co("close_lh", "p90_min"))` minutes, the
share travelling over 40 minutes rises to
`r fmt_pct(100 * co("close_lh", "over_40"), 0)`, and more children leave
the city: `r fmt_n(co("close_lh", "left_city"))` in 2030 against
`r fmt_n(co("today", "left_city"))`. It also lowers segregation more than
any other option in the table, to
`r sprintf("%.3f", co("close_lh", "gorard"))` against
`r sprintf("%.3f", co("today", "gorard"))`: Longhill's Year 7 intake is
among the most disadvantaged in the city, and its children would spread
across schools with less disadvantaged ones. That is a statistic about
intakes, not a benefit to the children who would travel further. It is the counterfactual against
which keeping Longhill open is a choice to spend money on shorter
journeys in the east.

## Fitting the system by 2035

```{r tbl-weigh-2035}
#| tbl-cap: "Removing places for 2035, when the cohort is at its lowest. Cutting only the community schools pushes children into the academies and faith schools, and the community schools lose; cutting every school in proportion keeps the children where they are. Council priorities, 2035."
fit <- c("today", "shrink", "shrink_all", "cn270_kings", "package_e", "close_lh")
cop$runs %>%
  filter(year == 2035, id %in% fit) %>%
  mutate(id = factor(id, fit)) %>% arrange(id) %>%
  transmute(Option = label,
            `Empty places` = fmt_n(empty),
            `Community schools' balance` = gbp_m(comm_gap),
            `City-wide balance` = gbp_m(city_gap),
            Displaced = fmt_n(displaced),
            `At a catchment school` = fmt_pct(100 * in_catchment, 0),
            `Longest journeys` = sprintf("%.0f min", p90_min)) %>%
  knitr::kable(align = "lrrrrrr")
```

By 2035 the city carries `r fmt_n(co("today", "empty", 2035))` empty
Year 7 places a year, `r fmt_n(co("today", "comm_empty", 2035))` of them
in the community schools. **Removing them from the community schools
alone makes the community schools worse off**: their balance goes from
`r gbp_m(co("today", "comm_gap", 2035))` to
`r gbp_m(co("shrink", "comm_gap", 2035))`, because the children they no
longer have room for go to the academies and faith schools, and
`r fmt_n(co("shrink", "displaced", 2035))` are displaced from their
catchment. **Removing places from every school in proportion** does the
opposite: `r gbp_m(co("shrink_all", "comm_gap", 2035))`, with
`r fmt_n(co("shrink_all", "displaced", 2035))` displaced and
`r fmt_pct(100 * co("shrink_all", "in_catchment", 2035), 0)` of children
at a catchment school. A viable system in 2035 is one in which the
schools the council does not run shrink with the ones it does, and the
council's case for asking them rests on exactly this comparison.

## All three together

**Where the priorities agree.**

- *Cardinal Newman and King's taking fewer* helps Priority 1 and
  Priority 3 together, and the community schools' finances most of all.
  It needs the dioceses' agreement, and the case to put to them is that a
  city whose cohort is falling cannot keep every school full, and that
  the schools it does not run are part of the same system.
- *Reviewing priority 6* touches little. It makes almost no difference to
  segregation and a small one to displacement, and it gives something
  real --- a route into the popular schools for the children of
  single-school catchments who use it. The case for keeping it at 5% is
  as good as the case against.

**Where they compete, and which should give.**

- *Longhill and journeys against mixed intakes.* Returning Whitehawk to
  Longhill's catchment gives Longhill
  `r fmt_n(co("whitehawk", "lh_intake") - co("today", "lh_intake"))` more
  children a year by 2030, lowers displacement and shortens the journeys
  of the children it moves --- and raises the segregation index from
  `r sprintf("%.3f", co("today", "gorard"))` to
  `r sprintf("%.3f", co("whitehawk", "gorard"))`, because the
  neighbourhoods it moves are `r fmt_pct(100 * whx$area$disadvantaged_share, 0)`
  disadvantaged against `r fmt_pct(100 * whx$area$city_share, 0)` across
  the city. Which way that trade should go is a judgement, and the
  council should make it knowing both halves --- and knowing that the
  higher index counts against the redraw under the council's aim of
  evening out intakes, not on the evidence about disadvantaged pupils'
  results (@sec-concentration-evidence).
- *Access against segregation.* Elm Grove all but closes the gap between
  deprived children's journeys and everyone else's, and it concentrates
  disadvantaged children in one school: the index rises from
  `r sprintf("%.3f", co("today", "gorard"))` to
  `r sprintf("%.3f", co("elm", "gorard"))`. A review of how to even out
  disadvantaged children's chances has to choose between those two
  measures, because relocation improves one and worsens the other. This
  document's evidence leans towards access: most of what separates
  schools' headline results arrives with the children rather than being
  added by the school (@sec-school-effects), so a more mixed intake is a
  weak lever on outcomes, while the journey is made by every child twice
  a day. For disadvantaged pupils the national evidence goes further:
  they do slightly better, not worse, where they are concentrated in a
  school organised for them, and Brighton Aldridge is the city's own
  example (@sec-concentration-evidence). What the document cannot show is
  whether shorter journeys raise attendance; the council's records could.
- *Longhill against short journeys.* Trimming the central schools helps
  Longhill for two or three years and lengthens journeys for the
  children moved. It should be done only with the Whitehawk redraw, and
  only while those schools are full.
- *Money against journeys.* Closing Longhill is the cheapest system and
  the one with the longest journeys in the east. Keeping it open is
  worth doing only as a deliberate choice to pay for shorter journeys and
  a school in the east, sized and staffed for the children it will get.
...
If the council's inequalities review puts evening out disadvantaged
children's chances first, the order of the recommendations changes. The
earlier version of this section treated a move to Elm Grove as an access
decision to be weighed against its cost to Longhill. On the full model
with the catchments redrawn, it is the option that does most for access
**and** most for Longhill's roll, and the map drawn round it settles the
Whitehawk question as part of a redesign. So it leads.

1. **Make a smaller Longhill at the top of Elm Grove the lead option, and
   test it properly.** At 150 places with the catchments redrawn round
   it, Longhill takes `r fmt_n(co("elm", "lh_intake", 2026))` children in
   2026 and `r fmt_n(co("elm", "lh_intake"))` in 2030, against
   `r fmt_n(co("today", "lh_intake", 2026))` and
   `r fmt_n(co("today", "lh_intake"))` today; deprived children stop
   travelling further than everyone else, a gap of
   `r sprintf("%.1f", max(0, co("elm", "dep_gap")))` minutes against
   `r sprintf("%.1f", co("today", "dep_gap"))`; Whitehawk's disadvantaged
   children's journeys fall from
   `r sprintf("%.0f", whe("today", "minutes_disadvantaged"))` to
   `r sprintf("%.0f", whe("elm", "minutes_disadvantaged"))` minutes; and the
   new map puts `r cop$elm_moves$whitehawk_to_longhill` of the
   `r cop$elm_moves$whitehawk_zones` Whitehawk neighbourhoods back in
   Longhill's catchment. A move needs a new map in any case, so
   Whitehawk's return comes as part of a redesign built round a new
   school rather than as a stand-alone reversal of the 2024 change. The
   costs belong in the same paper: a site, not costed here; a segregation
   index of `r sprintf("%.3f", co("elm", "gorard"))` against
   `r sprintf("%.3f", co("today", "gorard"))`;
   `r fmt_n(cop$elm_moves$children)` children a year in neighbourhoods
   that change catchment; longer journeys for Rottingdean, Saltdean and
   Woodingdean, more of whose children leave the city
   (`r fmt_n(co("elm", "left_city"))` from across the city in 2030
   against `r fmt_n(co("today", "left_city"))`); and a school that still
   does not break even.
2. **If the site cannot be had, return Whitehawk to Longhill's catchment
   on its own.** It gives Longhill
   `r fmt_n(co("whitehawk", "lh_intake") - co("today", "lh_intake"))`
   children a year by 2030 and lowers displacement, and it raises the
   segregation index from `r sprintf("%.3f", co("today", "gorard"))` to
   `r sprintf("%.3f", co("whitehawk", "gorard"))`. It also does little
   for Whitehawk's journeys while the school is at Ovingdean: its
   disadvantaged children travel
   `r sprintf("%.0f", whe("whitehawk", "minutes_disadvantaged"))` minutes
   against `r sprintf("%.0f", whe("today", "minutes_disadvantaged"))` now.
   It is the fallback, not the first choice.
3. **Set Longhill's admission number at 150**, on either site, and plan
   its budget and staffing for the children it will get --- about a
   hundred at Ovingdean, somewhat more at Elm Grove --- not for 150. Put
   the gap between that and break-even --- about
   `r gbp_m(-co("today", "lh_gap", 2030))` a year at Ovingdean in 2030 and
   `r gbp_m(-co("elm", "lh_gap", 2030))` at Elm Grove --- to members as
   the cost of keeping a school in the east, and decide it as that.
4. **Ask Cardinal Newman to reduce to 300, and King's to 150**, and
   Brighton Aldridge to 150, and make the case for each school in the
   city giving up places in proportion by 2030. With Longhill at Elm
   Grove these asks add most to its roll.
5. **Trim Stringer, Varndean and Blatchington Mill only if Longhill stays
   at Ovingdean and only with a catchment redraw** --- to 300, 270 and
   300, for the next two or three rounds while they are full, with
   Whitehawk returned to Longhill's catchment. With
   Longhill at Elm Grove the trims add few children in the model's runs
   and raise displacement.
6. **Publish where priority 6's places went**, so what it does to
   segregation can be measured rather than modelled.
7. **Ask the inequalities review to settle the measure.** Relocation
   shortens disadvantaged children's journeys and concentrates them in
   one school. Whether evening out chances means the first or the
   second, and whether shorter journeys raise attendance, are questions
   the review can answer from the council's own records and this
   document cannot. It should weigh the national evidence that
   disadvantaged pupils do no worse --- slightly better --- in schools
   where they are concentrated and well served
   (@sec-concentration-evidence), before treating a more concentrated
   intake as a cost.

Together --- Longhill at Elm Grove at 150 with the catchments redrawn,
Cardinal Newman at 300, King's at 150 and Brighton Aldridge at 150 ---
that package gives Longhill
`r fmt_n(co(pk, "lh_intake", 2026))` children in 2026 and
`r fmt_n(co(pk, "lh_intake", 2030))` in 2030, against
`r fmt_n(co("today", "lh_intake", 2026))` and
`r fmt_n(co("today", "lh_intake", 2030))`; a segregation index of
`r sprintf("%.3f", co(pk, "gorard"))` against
`r sprintf("%.3f", co("today", "gorard"))`;
`r fmt_n(co(pk, "displaced", 2026))` children displaced in 2026 against
`r fmt_n(co("today", "displaced", 2026))`;
`r fmt_pct(100 * co(pk, "in_catchment"), 0)` at a catchment school
against `r fmt_pct(100 * co("today", "in_catchment"), 0)`; and, in 2026,
longest journeys of `r sprintf("%.0f", co(pk, "p90_min", 2026))` minutes
against `r sprintf("%.0f", co("today", "p90_min", 2026))`; and a
deprived-journey gap of `r sprintf("%.1f", max(0, co(pk, "dep_gap")))` minutes
against `r sprintf("%.1f", co("today", "dep_gap"))`. It does not make
Longhill break even; that needs a decision about money. Without the
move, the best package tested --- Whitehawk back to Longhill, Longhill
at 150, the central trims, no priority 6, Cardinal Newman at 300 and
Brighton Aldridge at 150 --- gives Longhill
`r fmt_n(co("package_e", "lh_intake", 2026))` in 2026 and
`r fmt_n(co("package_e", "lh_intake"))` in 2030, and leaves the
deprived-journey gap at `r sprintf("%.1f", co("package_e", "dep_gap"))`
minutes: only a school nearer to them, or better buses, closes that
(@sec-decisions).

::: {.callout-warning appearance="simple"}
## What this rests on

Every number here is from a model calibrated to published catchment
totals, not to individual applications. The finances are a steady state
five years out, with a school shedding three quarters of the cost of a
pupil it loses. The catchment terms are carried to redrawn maps at the
strength families follow the maps they have, which is what decides every
redraw result above. Segregation counts disadvantaged pupils placed by
each school's fitted pull on them, which reproduces the published shares
for the cohorts admitted before the 2024 map and priorities, and assumes
those pulls hold under today's rules and when maps and schools change.
Most options differ on the index by a few thousandths, which is inside
what a change of measure has moved. And the
academies and faith schools are modelled as responding to demand, not as
making their own decisions about numbers.
The Elm Grove runs were corrected in this version: competition is now
measured from the Elm Grove site, and children refused elsewhere are
re-offered places with the second preferences families gave shifted by
how much the move changes each neighbourhood's pull towards the school.
Both raised Elm Grove's figures a little. The site itself, and what
building on it would cost, are not modelled.
...
**The council's admissions records.** @sec-data-asks lists the
releases that would replace a modelled flow with an observed one
