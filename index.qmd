...
# One radio-button group per thing that can be shown. Longhill comes
# first, under each of the three models, because the change between them
# is the point; every other school follows under the full model only.
fm_groups <- fm$net %>%
  st_drop_geometry() %>%
  distinct(model, name) %>%
  mutate(is_lh = name == "Longhill High School",
         label = if_else(
           is_lh,
           paste0("Longhill — ", recode(model, !!!as.list(FM_LABELS))),
           short_sch(name))) %>%
  arrange(desc(is_lh), match(model, names(FM_LABELS)), label)

fm_net <- fm$net %>%
  left_join(fm_groups %>% select(model, name, label), by = c("model", "name"))

# One width scale across every group, so a thick line means the same
# number of children whichever school is showing.
FM_MAX <- max(fm_net$flow)
fm_w   <- function(x) 1.1 + 7.5 * sqrt(x / FM_MAX)

FM_COL <- c(Bus = "#2166ac", Walk = "#d95f02")

fm_sch <- schools_sf() %>% filter(name %in% fm_groups$name)
```
...
```{r fig-flow-map}
#| fig-cap: "The same flows, for every school and interactively. Use the control to switch school, and for Longhill to switch between the three models of section 7.4. Blue is a bus leg, orange a walking leg. These are modelled flows from an open-data model: no pupil record is drawn here."
...
fm_bb <- st_bbox(fm_net %>% filter(name == "Longhill High School"))
