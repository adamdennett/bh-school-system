...
a result withdrawn pending recalibration, for the reason set out in
@sec-weighing.
...
::: {.callout-important appearance="simple"}
## The intake figures here are withdrawn pending recalibration

The catchment figures above measure the neighbourhoods each catchment
contains, and stand. The intake figures below measure modelled intakes by
the neighbourhoods their children come from, and that measure does not
match the schools' published shares of disadvantaged pupils --- see the
check at the head of @sec-weighing. They are withdrawn until the model is
recalibrated.
:::
...
::: {.callout-important appearance="simple"}
## Withdrawn pending recalibration

This subsection's finding rests on the model's neighbourhood measure of
how deprived each school's intake is. That measure does not match the
schools' published shares of disadvantaged pupils --- see the check at
the head of @sec-weighing --- so the result below is withdrawn until the
model is recalibrated. The mechanisms it describes may still operate;
their direction and size on the real intakes are not established.
:::
...
**On the model's measure, it makes the intakes more segregated, not
less** --- a result that rests on the neighbourhood measure @sec-weighing
finds does not match the schools' published disadvantaged shares, and
is withdrawn pending recalibration. Gorard's index
...
## A check the segregation results fail

```{r weigh-validate}
#| tbl-cap: "Each school's intake as the model measures it --- the share of its children from neighbourhoods in the three most deprived deciles --- against the Department for Education's published share of disadvantaged pupils at the school, in the latest year of the performance tables. The model's figures are for 2026 under the council's priorities. Ordered by the published share."
val_obs <- perf$bh %>%
  mutate(URN = as.character(URN)) %>%
  filter(!is.na(PTFSM6CLA1A), year_label == perf$latest) %>%
  inner_join(oi$schools %>% transmute(name, URN = as.character(urn)), by = "URN") %>%
  select(name, published = PTFSM6CLA1A)
val <- cms$schools %>% filter(id == "today") %>%
  transmute(name, short, intake, model = 100 * dep_share) %>%
  inner_join(val_obs, by = "name") %>%
  mutate(model_avg = weighted.mean(model, intake), pub_avg = weighted.mean(published, intake),
         model_side = ifelse(model >= model_avg, "above", "below"),
         pub_side = ifelse(published >= pub_avg, "above", "below"))
val_r <- cor(val$model, val$published)
val_r_nobaca <- with(val %>% filter(short != "Brighton Aldridge"), cor(model, published))
val_same <- sum(val$model_side == val$pub_side)
val_of <- function(s, col) val[[col]][val$short == s]
val %>%
  arrange(desc(published)) %>%
  transmute(School = short,
            `Model: from deprived neighbourhoods` = fmt_pct(model, 0),
            `Relative to the city` = model_side,
            `Published: disadvantaged pupils` = fmt_pct(published, 0),
            `Relative to the city ` = pub_side) %>%
  knitr::kable(align = "lrlrl")
```

Every segregation figure in this document measures how mixed a school's
intake is by the **neighbourhoods** its children come from. Checked
against the share of **disadvantaged pupils each school actually has**,
as the Department for Education publishes it, that measure gets the
schools in the wrong order. The two correlate at
`r sprintf("%.2f", val_r)`, and only because of Brighton Aldridge:
without it the correlation is `r sprintf("%.2f", val_r_nobaca)`. Only
`r val_same` of the `r nrow(val)` schools are on the same side of the
city average on both. Longhill and Hove Park, which the model puts below
the city average, have among the highest disadvantaged shares in the
city --- `r fmt_pct(val_of("Longhill", "published"), 0)` and
`r fmt_pct(val_of("Hove Park", "published"), 0)` --- and Varndean, which
the model puts well above it, has about the average.

The gap is consistent with families sorting *within* neighbourhoods: the
better-off families in a mixed neighbourhood choosing a different school
from their neighbours, which a neighbourhood measure cannot see. It is
also the process segregation is made of.

**Every segregation result in this document is therefore withdrawn
until the model is recalibrated against the published figures**: the
Gorard indices in this section, the intake results in section 8, the
priority-6 finding (@sec-fr-p6) and CoMArt's (@sec-comart). In
particular, the model's result that returning Whitehawk to Longhill's
catchment lowers segregation is **not supported**. On the published
figures that redraw would move children from a school with about the
city's share of disadvantaged pupils to one with well above it, which is
more likely to raise segregation than to lower it. The journey,
displacement, catchment and Longhill results below do not rest on this
measure and stand.
:::
...
```{r tbl-weigh-2030}
#| tbl-cap: "The options in 2030, under the council's priorities. Community schools' balance is the steady-state annual balance across the six schools the council runs (section 6). Gorard is across all city schools' intakes, on the model's neighbourhood measure, and is withdrawn pending recalibration (see the check at the head of this section). The deprived-journey gap is the extra minutes children from the three most deprived deciles travel. Displaced children would have taken a place in their own catchment and could not. Where an option cuts a school's admission number and that school is no longer full in 2030, the cut changes nothing that year."
...
- **Whitehawk back in Longhill's catchment** adds
  ...
  shows it lowers displacement. What it does to segregation is the open
  question in the check above.
...
`r sprintf("%.1f", co("comart", "dep_gap"))`. Nothing else moves it by
more than a minute.
...
**On the model's neighbourhood measure, Whitehawk back to Longhill gives
the lowest index of any option the council controls**,
`r sprintf("%.3f", co("whitehawk", "gorard"))`. That is the result the
check at the head of this section does not support: on the published
disadvantaged shares, the redraw moves children towards the more
disadvantaged intake, not away from it.

**On the same measure, priority 6 works against it.** Switched off, the index is
`r sprintf("%.3f", co("p6_off", "gorard"))`; at 20% it is
`r sprintf("%.3f", co("p6_20", "gorard"))` ([the priority-6 subsection of section 8](#sec-fr-p6)). Removing all
oversubscription priorities gives
`r sprintf("%.3f", co("equal", "gorard"))` but displaces
`r fmt_n(co("equal", "displaced", 2026))` children in 2026 against
`r fmt_n(co("today", "displaced", 2026))`.

**The power diagram does not help on this model.** Section 8 ranks it
the most even design under an in-catchment-then-distance allocation;
on the full model, with families' attachment to their own neighbourhood
carried to the new map, its index is
`r sprintf("%.3f", co("power", "gorard"))` and it costs Longhill
`r fmt_n(co("today", "lh_intake") - co("power", "lh_intake"))` children.
The two models disagree, and the disagreement turns on how closely
families would follow a redrawn boundary, which no published data shows.
...
`r fmt_n(co("today", "left_city"))`. It is the counterfactual against
...
- *Whitehawk back to Longhill* helps Priorities 1 and 3: more children
  for Longhill, less displacement, and no real change to journeys. What
  it does to segregation is not settled: the model says it lowers it,
  and the published disadvantaged shares suggest it would raise it.
...
- *Reviewing priority 6* helps Priority 2 and Priority 3 together. It
  costs something real --- a route into the popular schools for the
  children of single-school catchments who use it --- and that is a
  judgement the council should make knowing the segregation result.

**Where they compete, and which should give.**

- *Access against segregation.* Elm Grove and CoMArt shorten deprived
  children's journeys and, on the model's measure, concentrate them in
  one school --- a trade whose second half the recalibration has to
  confirm. If the council
...
1. **Test returning Whitehawk to Longhill's catchment** before proposing
   it. It gives Longhill children and lowers displacement; whether it
   helps or harms the social mix of intakes needs the recalibrated
   model, and on the published figures it may harm it.
...
`r fmt_n(co("today", "lh_intake", 2026))` and
`r fmt_n(co("today", "lh_intake", 2030))`;
...
redraw result above. And the academies and faith schools are modelled as
responding to demand, not as making their own decisions about numbers.
