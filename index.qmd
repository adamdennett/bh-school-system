...
| $W_j$ | how much *wanted* school there is at $j$, from expressed preferences |
  ...
  ui.innerHTML =
    ...
    '<div style=\"margin-top:6px;padding-top:6px;border-top:1px solid #eee\">' +
      '<label><input type=\"checkbox\" id=\"perchild\"> ' +
        'Divide by children living there</label></div>' +
...
::: {.callout-note}
## Why attractiveness is measured from preferences, not admission numbers

An admission number is set by the authority. It says how many places
exist, not whether anyone wants them. Longhill has the third-largest
admission number in the city and the fewest first preferences of any
school --- on a PAN-based weighting it would count as one of the most
attractive destinations in Brighton, which is plainly wrong.
...
them. In the two paired catchments a family has to rank the two local
schools against each other, so the catchment's first preferences are
split between them while its second preferences accumulate. The
signature is unmistakable:

```{r tbl-attractiveness}
#| tbl-cap: "Attractiveness weights. The ratio of second to first preferences separates the paired catchments from the single-school ones almost perfectly."

acc$attract %>%
  left_join(oi$schools %>% select(name, catchment), by = "name") %>%
  mutate(kind = case_when(
           is.na(catchment) ~ "Faith, city-wide",
           catchment %in% c("DS_Varndean", "Hove_Blatch") ~ "Paired catchment",
           TRUE ~ "Single-school catchment")) %>%
  arrange(desc(W_pref)) %>%
  transmute(School = name, Catchment = kind, PAN = pan,
            `1st prefs` = ifelse(imputed, "--", sprintf("%.0f", p1)),
            `2nd prefs` = ifelse(imputed, "--", sprintf("%.0f", p2)),
            `2nd / 1st` = ifelse(imputed, "--", sprintf("%.2f", p2 / p1)),
            `W (admission no.)` = sprintf("%.2f", W_pan),
            `W (preferences)` = sprintf("%.2f", W_pref)) %>%
...
and 0.6. Counting first preferences alone would mark down precisely the
schools whose catchment obliges families to put them second --- Hove
Park and Dorothy Stringer most of all.

Two limitations follow, and both matter more than the choice of
$\alpha$. **Peacehaven Community School is in Lewes district** and does
not appear in the council's factsheets, so it has no preference data;
its weight is imputed from its admission number at the city-average
rate. And the count is deliberately **not** divided by the admission
number, because what should be within reach is *places families want*,
which scales with the size of the school.

**The surface is not sensitive to any of this.** Recomputed on
admission-number weights instead, it correlates with the version mapped
here at
`r sprintf("%.2f", acc$w_check$spearman)`, with
`r fmt_pct(100 * acc$w_check$within_one, 0)` of neighbourhoods within one
decile. The weighting changes individual schools a great deal and the
map very little, because the measure sums over eleven of them.
:::
