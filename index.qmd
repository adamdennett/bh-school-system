...
```{r tbl-shape-audit}
#| tbl-cap: "Whether each design is a usable map. A fragment is a piece of a catchment detached from the rest of it. An enclave is a catchment wholly surrounded by one other, which a contiguity check misses because a ring is connected. The last column counts schools that fall outside their own catchment. The current map is scored on its whole-LSOA approximation, which is why it shows faults the published boundaries do not have."
...
i_elm  <- is_ %>% filter(str_detect(design, "Elm Grove"))
...
```{r tbl-idaci-deciles}
#| tbl-cap: "The share of each catchment's households with dependent children falling in each IDACI decile, under each design. Decile 1 is the most deprived tenth of neighbourhoods in England. Rows sum to 100%."

fr$idaci_table %>%
  mutate(design = factor(design, fr_designs), region = fr_lab(grp)) %>%
  arrange(design, region) %>%
  mutate(share = sprintf("%.0f", 100 * share)) %>%
  pivot_wider(id_cols = c(design, region), names_from = idaci_decile,
              values_from = share) %>%
  rename(Design = design, Catchment = region) %>%
  knitr::kable(align = c("l", "l", rep("r", 10)))
```

```{r fig-idaci-spread}
#| fig-cap: "Each catchment's share of households with dependent children in the three most deprived deciles, by design. One point per catchment. A design whose points are bunched spreads disadvantage evenly; one with a point far to the right has a catchment carrying much more of it than the others."
#| fig-height: 4.2

fr$idaci_region %>%
  mutate(design = factor(design, rev(fr_designs)), region = fr_lab(grp)) %>%
  ggplot(aes(dep3, design)) +
  geom_line(aes(group = design), colour = "grey70", linewidth = 3, alpha = 0.5) +
  geom_point(aes(size = hh), colour = "#b2182b", alpha = 0.85) +
  ggrepel::geom_text_repel(aes(label = region), size = 2.5, seed = 1,
                           max.overlaps = 30, box.padding = 0.35) +
  scale_x_continuous(labels = scales::label_percent()) +
  scale_size_continuous(range = c(1.5, 6), guide = "none") +
  labs(x = "Households with dependent children in the three most deprived deciles",
       y = NULL,
       title = "How evenly each design spreads disadvantage",
       subtitle = "One point per catchment, sized by how many households it holds.",
...
fr$idaci_summary %>%
  transmute(Design = design,
            ...
            `Spread` = sprintf("%.0f points", 100 * spread),
...
It gets there by having no catchment above
`r fmt_pct(100 * i_pd$hi, 0)`, where the current map has one at
`r fmt_pct(100 * i_now$hi, 0)`.

**`r nrow(i_worse)` of the four alternatives are *worse* than the
current map, and `r nrow(i_same)` matches it**, which the obvious
summary statistic hides. Look at the spread column: the flow-paired
design has the narrowest range of any design here,
`r sprintf("%.0f", 100 * i_pair$spread)` points against
`r sprintf("%.0f", 100 * i_now$spread)` for the current map. On that
measure it is the clear winner. On Gorard it is
`r sprintf("%.3f", i_pair$gorard)` --- the same as the map it replaces.

The two disagree because **a narrow range says nothing about how many
households sit at each end**. The current map's most deprived catchment
is `r fmt_pct(100 * i_now$hi, 0)` deprived, which is extreme --- but it
is BACA, and BACA is small. Concentrating disadvantage in a small
catchment moves fewer households than spreading a moderate excess across
two large ones. Gorard weights by size and max-minus-min does not, which
is why the ranking here uses Gorard: it is the measure the rest of this
work uses for exactly this question.

::: {.callout-important appearance="simple"}
## A finding that reversed under a better measure

An earlier version of this section reported that **every** alternative
was less segregated than the current map. That was computed as the
range between the least and most deprived catchment, and on that
statistic it was true.

It does not survive being measured properly. Once catchment size is
taken into account, `r nrow(i_worse)` of the four alternatives spread
...
and it is better on capacity and journey time too.
:::

And a caution against reading any of this as a solution.
`r sprintf("%.3f", i_best$gorard)` is a large index. **No redrawing of a
catchment map fixes residential segregation**, because the segregation
is in where families live, not in where the lines are. What a map can
do is stop amplifying it, and the difference between the best and worst
design here is the size of that amplification.

## And if Longhill moves {#sec-fr-elm}

The last design re-runs everything for the configuration @sec-longhill
finds composes best: **Longhill at the top of Elm Grove with its
admission number cut to `r fr$elm_pan`**. The model is re-run on the
relocated travel matrix, the catchments are redrawn from those flows,
and the deprivation profile recomputed.

```{r fr-elm-stats}
#| include: false
a_now  <- fr$alloc %>% filter(str_detect(design, "Current"))
a_pair <- fr$alloc %>% filter(str_detect(design, "pairs kept"))
a_elm  <- fr$alloc %>% filter(str_detect(design, "Elm Grove"))
a_pd   <- fr$alloc %>% filter(str_detect(design, "Power"))
elm_moved <- fr$changed %>% filter(str_detect(design, "Elm Grove"))
ec <- fr$east_check
ec_near <- names(sort(table(ec$nearest), decreasing = TRUE))[1]
ec_to   <- names(sort(table(ec$assigned), decreasing = TRUE))[1]
ec_t1 <- weighted.mean(ec$t_near, ec$Oi); ec_t2 <- weighted.mean(ec$t_assigned, ec$Oi)
```

It gives the **best capacity balance of the flow designs**,
`r sprintf("%+.0f%%", 100 * d_elm$worst_gap)`, and it places
`r fmt_pct(100 * a_elm$local_share, 0)` of children in their own
catchment, the highest of any design. It also has the **longest
journeys**, `r sprintf("%.1f", d_elm$mean_journey)` minutes against
`r sprintf("%.1f", d_now$mean_journey)` now, and it moves
`r fmt_pct(100 * sum(elm_moved$Oi[elm_moved$moved]) / sum(elm_moved$Oi), 0)`
of the cohort into a different catchment --- much the largest upheaval
on offer.

::: {.callout-important appearance="simple"}
## The east of the city is the problem, and the map shows it

The Stringer/Varndean catchment in this design reaches from Varndean to
Saltdean. That is an odd-looking boundary and the temptation is to treat
it as a defect of the algorithm. It is not: it is the algorithm
reporting something real.

For **all `r nrow(ec)`** of the easternmost neighbourhoods, the
relocated Longhill is still the **most accessible** school ---
`r sprintf("%.0f", ec_t1)` minutes on average, because the coast road
reaches Elm Grove before it reaches Varndean. They are assigned to
`r fr_lab(ec_to)` instead, at `r sprintf("%.0f", ec_t2)` minutes: a
penalty of about **`r sprintf("%.0f", ec_t2 - ec_t1)` minutes each way**.

They are assigned there because there is nowhere else. At an admission
number of `r fr$elm_pan` the relocated Longhill is already full on its
own doorstep, and BACA is exactly at its capacity. The two schools the
east can reach have no room, so the east attaches to one it cannot.

**That is a finding about the configuration, not about the boundary.**
Moving Longhill west and shrinking it at the same time leaves the
eastern edge of the city without a school within reach. The map can
only record that; it cannot fix it. A relocation at a larger admission
number, or a relocation without one, would draw a different map --- and
this is the question a redesign should be asked before a boundary is.
:::

And on deprivation it is among the **weakest designs here**, with a
Gorard index of `r sprintf("%.3f", i_elm$gorard)` against
`r sprintf("%.3f", i_now$gorard)` for the map in force and
`r sprintf("%.3f", i_pd$gorard)` for the power diagram. Moving the
school west moves its catchment west, into the part of the city the
current map already separates out, and the boundary that results
reproduces much of that separation while making the catchment carrying
it larger.

So the relocation that @sec-longhill finds best for Longhill is the
redesign that spreads disadvantage least evenly and strands the east.
That is not an argument against moving the school. It is an argument
against letting the catchment map follow it without being drawn
deliberately, and for asking what happens to Ovingdean, Rottingdean and
Saltdean before rather than after.
