...
variance in Attainment 8 puts the part a school directly controls --- its
workforce --- at **`r ex_wf_all`%**, against **`r ex_ex_all`%** inherited
with the intake. For disadvantaged pupils, the group any mixing policy is
aimed at, the school-controllable share falls to **`r ex_wf_dis`%**. Most
of what separates one school's results from another's arrives with the
children, and a redistribution policy works on the smallest term while
leaving the rest untouched. Nor can the measured drivers be separated: of
the school-level variation deprivation, absence and prior attainment
jointly explain, about three quarters is variance they *share*. This is
not an argument that schools do not matter --- it is an argument that the
council should not expect admissions changes to move attainment much.
...
gets *smaller*, not larger, precisely where the policy is pointed.
...
arithmetic limits what it can deliver. And the limit is tightest for
disadvantaged pupils, where the school-controllable share is
`r wf_dis`%.
