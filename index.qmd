...
```{r fig-demand-change}
#| fig-cap: "Change in weighted preferences between the first and last five-year windows in the published series. Preferences are counted at all three ranks with the same geometric decay used in section 4, so a school that families consistently name second is not scored as though nobody wanted it."
#| fig-height: 5
...
ggplot(chg, aes(reorder(name, change), change, fill = change > 0)) +
  ...
  geom_text(aes(label = sprintf("%+.0f%%", pct),
                hjust = ifelse(change > 0, -0.15, 1.15)), size = 3.1) +
  coord_flip() +
  scale_y_continuous(expand = expansion(mult = 0.12)) +
  scale_fill_manual(values = c(`TRUE` = "#2166ac", `FALSE` = "#b2182b"),
                    guide = "none") +
  labs(x = NULL, y = "Change in average weighted preferences",
       title = "Where demand has moved under the catchment system",
       subtitle = sprintf(
         "Average of %d-%d against average of %d-%d, ranks 1-3 with decay %.2f.\nThe whole series post-dates the 2008 move to catchments with a lottery tie-break.",
         yr_rng[1], yr_rng[1] + 4, yr_rng[2] - 4, yr_rng[2], DECAY),
       caption = "Source: BHCC published allocation factsheets.") +
  theme_bh() +
  theme(panel.grid.major.y = element_blank())
```
...
#| fig-cap: "The same weighted preferences as a trajectory rather than a start-and-end comparison. Each school is indexed to 100 at its own five-year opening average, so the lines show relative movement rather than size."
#| fig-height: 6
...
ggplot(traj, aes(year, idx, colour = name)) +
  ...
  labs(x = NULL, y = "Weighted preferences, indexed to opening average = 100",
       title = "The same movement, seen as trajectories",
       subtitle = sprintf("Ranks 1-3 with decay %.2f. Above the dashed line is growth on the %d-%d baseline.",
...
### Does the answer depend on which preference you count? {#sec-choice-ranks}

The chart above counts first preferences. Section 4.5 argued that first
preferences alone distort the picture, because the paired catchments
force families to rank two local schools against each other. So it is
worth asking whether the finding survives counting the other ranks.

```{r rank-choice-data}
#| include: false

# Preference counts at each rank, five-year mean, joined to attainment
# and value added. The factsheets call it "Hove Park School" and the
# performance data "Hove Park School and Sixth Form Centre", so the join
# goes through the URN in oi$attract rather than the name.
rank_prefs <- fp$factsheets %>%
...
stopifnot(nrow(rank_prefs) == 10)

rank_dat <- os$choice %>%
  select(urn, school, pan, att8, va) %>%
  inner_join(rank_prefs %>% select(-name), by = "urn") %>%
  mutate(`First preferences`  = p1 / pan,
         `Second preferences` = p2 / pan,
         `Third preferences`  = p3 / pan,
         `Weighted (ranks 1-3)` = (p1 + DECAY_A * p2 + DECAY_A^2 * p3) / pan)

stopifnot(nrow(rank_dat) == 10)

MEASURES <- c("First preferences", "Second preferences",
              "Third preferences", "Weighted (ranks 1-3)")

rank_long <- rank_dat %>%
  select(school, att8, va, all_of(MEASURES)) %>%
  pivot_longer(all_of(MEASURES), names_to = "measure", values_to = "per_place") %>%
  pivot_longer(c(att8, va), names_to = "predictor", values_to = "x") %>%
  mutate(measure = factor(measure, MEASURES),
         predictor = factor(predictor, c("att8", "va"),
                            c("Attainment 8 (headline)",
                              "Value added (Lever report)")))

# R-squared for every measure-by-predictor pair, on ten schools.
rank_fit <- rank_long %>%
  group_by(measure, predictor) %>%
  summarise(r2 = summary(lm(per_place ~ x))$r.squared,
            slope = coef(lm(per_place ~ x))[2], .groups = "drop")
```

```{r fig-choice-ranks}
#| fig-cap: "Preferences per place at each rank, against headline attainment and against value added. Each point is a school; the dashed line is the fit and the figure in each panel is its R-squared."
#| fig-height: 8

rank_long %>%
  left_join(rank_fit, by = c("measure", "predictor")) %>%
  ggplot(aes(x, per_place)) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE,
              colour = "grey35", linewidth = 0.7, linetype = "22") +
  geom_point(aes(colour = predictor), size = 2.4, show.legend = FALSE) +
  geom_text(aes(label = sprintf("R² = %.2f", r2)),
            x = -Inf, y = Inf, hjust = -0.15, vjust = 1.4, size = 3.2,
            check_overlap = TRUE) +
  facet_grid(measure ~ predictor, scales = "free", switch = "y") +
  scale_colour_manual(values = c("Attainment 8 (headline)" = "#b2182b",
                                 "Value added (Lever report)" = "#2166ac")) +
  labs(x = NULL, y = "Preferences per place",
       title = "Families track the headline score at every rank",
       subtitle = "Ten schools, five-year means. Left column: headline attainment. Right: value added.",
       caption = "Sources: BHCC factsheets; DfE performance tables; How to Pull the Right Lever.") +
  theme_bh(11) +
  theme(strip.text.y.left = element_text(angle = 0, hjust = 0, face = "bold"),
        strip.placement = "outside")
```

```{r tbl-choice-ranks}
#| tbl-cap: "How much of each preference measure the two attainment measures explain. Ten schools, so these are descriptive rather than inferential."

rank_fit %>%
  mutate(r2 = sprintf("%.2f", r2)) %>%
  pivot_wider(id_cols = measure, names_from = predictor, values_from = r2) %>%
  rename(`Preference measure` = measure) %>%
  knitr::kable(align = "lrr")
```

```{r rank-stats}
#| include: false
att_lab <- "Attainment 8 (headline)"; va_lab <- "Value added (Lever report)"
g <- function(m, p) rank_fit$r2[rank_fit$measure == m & rank_fit$predictor == p]
r_p1 <- g("First preferences", att_lab)
r_p2 <- g("Second preferences", att_lab)
r_p3 <- g("Third preferences", att_lab)
r_pw <- g("Weighted (ranks 1-3)", att_lab)
v_p1 <- g("First preferences", va_lab)
v_pw <- g("Weighted (ranks 1-3)", va_lab)
```

The answer is that it survives, it strengthens, and there is one
exception worth dwelling on.

**It strengthens.** The weighted measure gives the *strongest*
association of all with headline attainment, R²
`r sprintf("%.2f", r_pw)` against `r sprintf("%.2f", r_p1)` for first
preferences alone. Counting the other ranks does not dilute the finding;
it sharpens it. So the association in the chart above is not an artefact
of looking only at firsts --- if anything, firsts understate it.

**Attainment's grip weakens down the ranks.** It explains
`r sprintf("%.2f", r_p1)` of first preferences,
`r sprintf("%.2f", r_p2)` of seconds and `r sprintf("%.2f", r_p3)` of
...
value added explains almost as much as attainment does ---
`r sprintf("%.2f", g("Second preferences", va_lab))` against
`r sprintf("%.2f", r_p2)`. On first preferences value added manages
`r sprintf("%.2f", v_p1)`; on thirds,
`r sprintf("%.2f", g("Third preferences", va_lab))`, which is nothing at
all.
...
# in turn.
loo_r2 <- vapply(seq_len(nrow(rank_dat)), function(i)
  summary(lm(rank_dat$`Second preferences`[-i] ~ rank_dat$va[-i]))$r.squared,
  numeric(1))
```
...
Every panel here is fitted on ten schools. That is enough to show an
association and nowhere near enough to separate attainment from the
things correlated with it --- intake, location, reputation, or the
Ofsted grade families may actually be reading. Nothing here identifies
what causes what.
...
establishing, because it is the obvious objection to the single chart
above, and it is the objection the council could most easily raise.
