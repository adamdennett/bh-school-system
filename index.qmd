...
ggplot(rp, aes(yr, reserve_pct)) +
  ...
  labs(x = NULL, y = "Revenue reserve as a share of annual income",
       ...
       caption = "Source: DfE school income and expenditure (CFR and academy accounts returns).") +
  theme_bh(10) +
...
The model puts `r fmt_n(cy_a$intake)` children into them in
`r cy_a$entry_year` and `r fmt_n(cy_z$intake)` in `r cy_z$entry_year`**
--- a fill rate falling from
`r fmt_pct(100 * cy_a$intake / cy_a$pan, 0)` to
`r fmt_pct(100 * cy_z$intake / cy_z$pan, 0)`, and
`r fmt_n(cy_z$pan - cy_z$intake)` empty places. Carried through to the
whole roll at each school's own funding rate, that is about
...
#| tbl-cap: "What the projected intakes are worth. The intake columns are modelled, not forecast: they are configuration A of the open scenarios — the city exactly as it stands. The income column applies each school's own income per pupil to the steady-state change in roll, in today's money. It is an order of magnitude, not a budget line."
#| column: page
...
fex %>%
  ...
  transmute(School = paste0(name, if_else(faith, " †", "")),
            `Reserve, % of income` = pp(reserve_pct),
            `Intake 2026` = fmt_n(intake_first),
            `Intake 2035` = fmt_n(intake_last),
...
Three of them are worth naming individually. **Cardinal Newman** is the
largest school in the city and already
`r pp(f_of("Cardinal Newman")$reserve_pct)` on its reserve, though its
intake column is the unreliable one. **Patcham** is at
`r pp(f_of("Patcham")$reserve_pct)` with a projected fall of
`r pp(f_of("Patcham")$intake_pct, 0)` --- worth
`r gbp(f_of("Patcham")$income_change)` a year, or
`r pp(f_of("Patcham")$income_pct, 0)` of its income. **Brighton
Aldridge** has spent its reserve down to
`r gbp(f_of("Brighton Aldridge")$reserve)` and faces the steepest projected fall
in intake of any school whose modelled intake can be trusted,
`r pp(f_of("Brighton Aldridge")$intake_pct, 0)`.
...
```{r longhill-rank}
#| include: false
# Superlatives here are computed rather than asserted. A draft of this
# paragraph called Longhill's the largest projected fall of any school;
# it is the fourth largest, and the largest only as a share of the
# school's own income - which is the figure that actually bears on
# whether it can carry the loss.
nf <- fex %>% filter(!faith)
stopifnot(
  # Longhill loses the largest share of its own income of any school in
  # the city, faith schools included.
  str_detect(fex %>% slice_min(income_pct, n = 1) %>% pull(name), "Longhill"),
  # Brighton Aldridge has the steepest fall in intake of the eight whose
  # modelled intake is worth reading.
  str_detect(nf %>% slice_min(intake_pct, n = 1) %>% pull(name), "Aldridge"),
  # And Hove Park, not Cardinal Newman, holds the worst reserve position
  # once it is put as a share of income - which is the comparison this
  # section makes throughout.
  str_detect(fex %>% slice_min(reserve_pct, n = 1) %>% pull(name), "Hove Park"))
```

**Longhill stands to lose more of its income than any other school**
(`r pp(f_of("Longhill")$income_pct, 0)`, or
`r gbp(f_of("Longhill")$income_change)` a year on a fall of
`r pp(f_of("Longhill")$intake_pct, 0)` in intake) **and it is not yet in
deficit.** It is the school with the most reserve left of the shrinking
ones and the least time to use it, which is the subject of the next
part of this section and of @sec-longhill.

**Hove Park is the counter-example that stops this being a story about
falling rolls.** Its modelled intake does not fall at all --- it is at
its admission number in `r cy_a$entry_year` and still at it in
`r cy_z$entry_year` --- and it has the worst reserve position in the
city as a share of income. Its roll has already fallen, by
`r fmt_n(abs(fch$roll_change[str_detect(fch$name, "Hove Park")]))` in
four years, and its costs have not followed. The damage is done rather
than coming.

::: {.callout-warning appearance="simple"}
## Four things this arithmetic is not
...
**The intakes are modelled.** They come from the open model at
configuration A --- the city as it stands, with today's admission
numbers held constant to 2035. That is the point of the exercise, not a
prediction: it is what happens *if nothing is decided*. The council is
already reducing admission numbers, which changes every column.
...
ggplot(bd, aes(share, row, fill = band)) +
  ...
  geom_text(data = lab, aes(x = 1.03, y = row,
            ...
            hjust = 0, size = 2.9, colour = "grey25", inherit.aes = FALSE) +
