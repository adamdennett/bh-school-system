...
show <- c("today", "lh150", "central", "whitehawk", "whitehawk_central", "elm",
          ...
          "equal", "power", "comart", "package_d", "package_e", "close_lh")
cop$runs %>%
...
- **Moving to Elm Grove with redrawn catchments** adds
  `r fmt_n(co("elm", "lh_intake") - co("today", "lh_intake"))`, at the
  cost of a new site and of the segregation result under Priority 2.
...
- *Access against segregation.* Elm Grove shortens deprived children's
  journeys and concentrates them in one school; CoMArt shortens them
  with little change to how mixed intakes are. If the council
  values access more, relocation is its strongest tool; if it values
  mixed intakes more, it should not move Longhill for that reason.
...
1. **Consider returning Whitehawk to Longhill's catchment.** Returning it
   to Longhill's catchment gives Longhill
   `r fmt_n(co("whitehawk", "lh_intake") - co("today", "lh_intake"))`
   children a year by 2030 and
   lowers displacement, and gives the children who take a Longhill place
   a shorter journey. Its effect on how mixed intakes are is small and
   not reliably in either direction; the case rests on Longhill and on
   journeys, not on segregation.
2. **Set Longhill's admission number at 150**, and plan its budget and
   staffing for about a hundred children a year, not for 150. Put the
   gap between that and break-even --- about
   `r gbp_m(-co("today", "lh_gap", 2030))` a year on the intake it would
   have in 2030 with nothing changed ---
   to members as the cost of keeping a school in the east, and decide it
   as that.
3. **Trim Stringer, Varndean and Blatchington Mill** --- to 300, 270
   and 300 --- for the next two or three rounds, while they are full,
   alongside the redraw, and not without it.
4. **Ask Cardinal Newman to reduce to 300, and King's to 150**, and
   ...
   city giving up places in proportion by 2030.
5. **Publish where priority 6's places went**, so what it does to
   segregation can be measured rather than modelled.
6. **Do not move Longhill to raise its roll**; consider Elm Grove only as
   an access decision for deprived children in the east, with the
   segregation cost stated.

Together --- Whitehawk back to Longhill, Longhill at 150, the central
trims, priority 6 switched off (the far end of reviewing it), Cardinal
Newman at 300 and Brighton Aldridge at 150 --- that package gives Longhill
...
against `r sprintf("%.0f", co("today", "p90_min", 2026))`. It does not make Longhill
break even, and it does not shorten deprived children's journeys. The
first needs a decision about money; the second needs a decision about
where schools are, or about buses (@sec-decisions).
