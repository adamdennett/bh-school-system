...
#| fig-cap: "Revenue reserve as a share of annual income, for every state secondary school in the city. Panels are ordered by the latest figure, worst first. Shading marks the years a school was carrying a deficit; hover a point for the cash figure. The academies' series ends a year earlier because their accounts are published on a different cycle — see the note below the table above."
#| column: page
...
rp <- fpan %>%
  mutate(school = factor(short_sch(name), short_sch(ord)),
         ...
         hov = sprintf("%s, %s<br>Reserve %s (%s of income)<br>In-year %s<br>Roll %s",
                       short_sch(name), year_label, gbp(reserve),
                       pp(reserve_pct), gbp(balance), fmt_n(roll)))

p <- ggplot(rp, aes(yr, reserve_pct, group = school, text = hov)) +
  geom_hline(yintercept = 0, colour = "grey40", linewidth = 0.4) +
  geom_ribbon(aes(ymin = neg, ymax = 0), fill = DEFICIT_COL, alpha = 0.28) +
  geom_line(linewidth = 0.8, colour = "grey20") +
  geom_point(size = 1.6, colour = "grey20") +
...
# Drawn live rather than as a PNG, so the panels stay sharp at any zoom
# and each point can carry the cash figure behind the percentage.
plotly::ggplotly(p, tooltip = "text", height = 560) %>%
  plotly::layout(
    margin = list(t = 60),
    title = list(
      text = paste0("Every school but one has been spending its reserve",
                    "<br><span style='font-size:12px;color:#555'>",
                    "Shaded years are years the school was carrying a deficit. ",
                    "The line is the accumulated surplus, not the in-year balance.",
                    "</span>"),
...
*Source: DfE school income and expenditure --- Consistent Financial
Reporting returns for maintained schools and Academies Accounts Returns
for academies, both published on the*
*[schools financial benchmarking service](https://financial-benchmarking-and-insights-tool.education.gov.uk/).*
...
```{r fig-risk-gap}
#| fig-cap: "The annual gap between what each school receives and what it spends, projected to 2035 on the middle assumption that a school takes out three quarters of the cost when it loses the funding. The gap starts at each school's most recent in-year balance and deteriorates as the roll falls. Schools that end below zero are coloured; hover for the figures behind any point."
#| column: page

rgap <- risk_mid %>%
  mutate(ends_down = short %in% r_neg$short,
         hov = sprintf("%s, %d<br>Roll %s (%+.0f on 2026)<br>Funding lost %s<br>Annual gap %s (%s of income)",
                       short, year, fmt_n(roll), d_roll,
                       gbp(lost_funding), gbp(gap), pp(gap_pct)))

pal <- setNames(c("#d03b3b", "#eb6834", "#c2701c", "#6b4fbb", "#2a78d6", "#1baf7a")[
                  seq_len(nrow(r_neg))],
                r_neg$short[order(r_neg$gap)])

p <- plotly::plot_ly(height = 520)
for (s in unique(rgap$short[!rgap$ends_down]))
  p <- p %>% plotly::add_lines(
    data = rgap %>% filter(short == s), x = ~year, y = ~gap, name = s,
    line = list(color = "#c8c8c8", width = 1.6), showlegend = FALSE,
    hovertext = ~hov, hoverinfo = "text")
for (s in names(pal))
  p <- p %>% plotly::add_lines(
    data = rgap %>% filter(short == s), x = ~year, y = ~gap, name = s,
    line = list(color = unname(pal[s]), width = 2.6),
    hovertext = ~hov, hoverinfo = "text")

p %>%
  plotly::layout(
    shapes = list(list(type = "line", x0 = min(RISK_YEARS), x1 = max(RISK_YEARS),
                       y0 = 0, y1 = 0, line = list(color = "#555", width = 1))),
    xaxis = list(title = "", dtick = 2, zeroline = FALSE),
    yaxis = list(title = "Annual surplus or deficit", tickprefix = "£",
                 tickformat = ".2s", zeroline = FALSE),
    legend = list(orientation = "h", x = 0, y = -0.12, font = list(size = 11)),
    margin = list(t = 90, r = 20),
    # Counted, not asserted: a draft of this title said three schools
    # were already below the line when five of them were.
    title = list(
      text = paste0(
        knitr::combine_words(c(
          sprintf("%d schools end below the line", nrow(r_neg)),
          sprintf("%d are already there", sum(rgap$gap[rgap$year == min(RISK_YEARS)] < 0)))),
        "<br><span style='font-size:12px;color:#555'>",
        "Grey lines are the schools that stay in surplus. ",
        "Middle assumption: a school takes out three quarters of the ",
        "cost when the funding goes.</span>"),
      x = 0, xanchor = "left", font = list(size = 15)),
    hoverlabel = list(align = "left")) %>%
  plotly::config(displayModeBar = FALSE)
```

*Sources: DfE school income and expenditure; DfE school funding
statistics 2025-26; BHCC allocation factsheets; modelled intakes,
configuration A of the open scenarios.*
...
```{r fig-idaci-bands}
#| fig-cap: "How each catchment's households with dependent children divide between the most deprived three deciles nationally, the middle four, and the least deprived three. Every bar is one catchment, grouped by design and ordered within it by how deprived the catchment is. The figure at the right of each bar is its share in the three most deprived deciles; hover a segment for the household count behind it."
#| column: page
...
bd <- bd %>% mutate(row = paste(design, region, sep = "|"))
row_lv <- unique(bd$row)

# A blank row between designs, carrying the design's name. It does the
# job the facet strips used to do, without needing facets - which is
# what makes this one set of axes and so a single live plot.
spacers <- tibble(design = fr_designs, row = paste0("~~", fr_designs))
ordered <- unlist(lapply(fr_designs, function(d)
  c(spacers$row[spacers$design == d], row_lv[startsWith(row_lv, paste0(d, "|"))])))

# plotly stacks categorical y axes from the bottom, so the order is
# reversed to read top-down.
lev <- rev(ordered)
lab <- setNames(sub("^.*\\|", "", ordered), ordered)
lab[startsWith(ordered, "~~")] <- ""

bd <- bd %>% mutate(row = factor(row, lev))

p <- plotly::plot_ly(height = 26 * length(ordered) + 170)
for (b in fr$idaci_band_levels) {
  s <- bd %>% filter(band == b)
  p <- p %>% plotly::add_trace(
    data = s, x = ~share, y = ~row, type = "bar", orientation = "h",
    name = b, marker = list(color = unname(BAND_COL[b]),
                            line = list(color = "white", width = 1)),
    hovertemplate = paste0(
      "<b>%{customdata[0]}</b><br>%{customdata[1]}<br>",
      b, ": %{x:.0%} (%{customdata[2]} households)<extra></extra>"),
    customdata = ~cbind(region, as.character(design), fmt_n(hh)))
}

dep_lab <- bd %>% filter(band == fr$idaci_band_levels[1])

p %>%
  plotly::layout(
    barmode = "stack", bargap = 0.28,
    xaxis = list(title = "Households with dependent children",
                 tickformat = ".0%", range = c(0, 1.16),
                 dtick = 0.25, zeroline = FALSE),
    yaxis = list(title = "", categoryorder = "array", categoryarray = lev,
                 tickvals = ordered, ticktext = unname(lab),
                 tickfont = list(size = 11)),
    legend = list(orientation = "h", x = 0, y = 1.04, font = list(size = 11)),
    margin = list(l = 170, t = 90),
    title = list(
      text = paste0("What kind of neighbourhood each catchment holds",
                    "<br><span style='font-size:12px;color:#555'>",
                    "Each bar is one catchment, ordered within its design by ",
                    "how deprived it is.</span>"),
      x = 0, xanchor = "left", font = list(size = 15)),
    annotations = c(
      # The share in the most deprived band, printed clear of the bar.
      # Inside it sat on the darkest fill and could not be read.
      lapply(seq_len(nrow(dep_lab)), function(i) list(
        x = 1.02, y = as.character(dep_lab$row[i]), xref = "x", yref = "y",
        text = sprintf("%.0f%%", 100 * dep_lab$share[i]),
        showarrow = FALSE, xanchor = "left",
        font = list(size = 11, color = "#3d3d3d"))),
      # The design names, in place of facet strips.
      lapply(seq_len(nrow(spacers)), function(i) list(
        x = 0, y = spacers$row[i], xref = "x", yref = "y",
        text = paste0("<b>", str_wrap(spacers$design[i], 40), "</b>"),
        showarrow = FALSE, xanchor = "left", yanchor = "middle",
        font = list(size = 11, color = "#1a1a1a")))))  %>%
  plotly::config(displayModeBar = FALSE)
```

*Sources: ONS postcode household counts; IMD 2019 IDACI.*
