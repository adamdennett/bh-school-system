...
**The proximity design wins most of this, and that was not what the
exercise expected.** The power diagram more than halves the worst
capacity gap, from `r sprintf("%+.0f%%", 100 * d_now$worst_gap)` to
`r sprintf("%+.0f%%", 100 * d_pd$worst_gap)`, improves self-containment
a little, adds `r sprintf("%.1f", d_pd$mean_journey - d_now$mean_journey)`
minutes to the average journey, and does it while moving the fewest
neighbourhoods of any design here.

**The flow regions win the statistic they were built to win**, and only
that one. Keeping the council's pairings gives the best self-containment
of any design, `r fmt_pct(100 * d_pair$self_containment, 0)` against
`r fmt_pct(100 * d_now$self_containment, 0)` now --- but at
`r sprintf("%.1f", d_pair$mean_journey - d_now$mean_journey)` extra
minutes and a worse capacity balance than the power diagram. Boundaries
drawn round where children *want* to go are not boundaries that get them
there quickly.

**Splitting the paired catchments fails.** One region per school gets
capacity to `r sprintf("%+.0f%%", 100 * d_one$worst_gap)` but leaves
self-containment exactly where the current map has it, at
`r fmt_pct(100 * d_one$self_containment, 0)`. That is the flows saying
what the council already knows: Dorothy Stringer and Varndean are 470
metres apart, and no boundary between them corresponds to anything
families do. On the map that design is the one where Dorothy Stringer
becomes a thin strip along the coast.
...
i_worse <- fr$idaci_summary %>% filter(gorard > i_now$gorard)
```
...
**Two of the four alternatives are *worse* than the current map**, and
the reason is worth understanding, because the obvious summary statistic
hides it. Look at the spread column: the flow-paired design has the
narrowest range of any design here,
`r sprintf("%.0f", 100 * i_pair$spread)` points against
`r sprintf("%.0f", 100 * i_now$spread)` for the current map. On that
measure it looks like the clear winner. On Gorard it is
`r sprintf("%.3f", i_pair$gorard)`, second worst.
...
`r nrow(i_worse)` of the four alternatives spread disadvantage *less*
evenly than the map in force once catchment size is taken into account.
The claim has been withdrawn and the ranking above is by Gorard.
...
Relocating the school and redrawing round it gives the **best capacity
balance of any design here**,
`r sprintf("%+.0f%%", 100 * d_elm$worst_gap)`, and lifts
self-containment to `r fmt_pct(100 * d_elm$self_containment, 0)`. It
also has the **longest journeys** of any design,
`r sprintf("%.1f", d_elm$mean_journey)` minutes against
...
And on deprivation it is the **weakest design here**, with a Gorard
index of `r sprintf("%.3f", i_elm$gorard)` against
...
current map already separates out, and the new boundary reproduces much
of that separation while making the catchment that carries it larger.

So the relocation that @sec-longhill finds best for Longhill is the
redesign that spreads disadvantage least evenly. That is not an argument
against moving the school. It is an argument for not letting the
catchment map follow it without being drawn deliberately.
