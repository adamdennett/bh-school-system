...
# Both measures share the same label, so a reader switching tabs can see
# the two numbers for whichever neighbourhood they are hovering over.
acc_lab <- sprintf(
  "%s | gravity index %.0f | %.0f places within 30 min | nearest %s, %.0f min",
  acc_map$lsoa21nm, acc_map$A_index, acc_map$places_30,
  acc_map$nearest_school, acc_map$nearest_min)

pal_grav <- colorNumeric("viridis", domain = acc_map$A_index, reverse = TRUE)
pal_cum  <- colorNumeric("viridis", domain = acc_map$places_30, reverse = TRUE)
...
```{r fig-acc-gravity}
#| fig-cap: "Gravity accessibility to secondary school places. Indexed so the child-weighted city average is 100. Darker means less school within reach."
#| fig-height: 6

acc_base() %>%
  addPolygons(data = acc_map, fillColor = ~pal_grav(A_index),
              fillOpacity = 0.8, color = "white", weight = 0.3,
              label = acc_lab) %>%
  addLegend(pal = pal_grav, values = acc_map$A_index,
            title = "Gravity index<br>(city average = 100)",
            position = "bottomright", opacity = 0.85)
```
...
#| fig-cap: "School places reachable within 30 minutes by walking and bus. Darker means fewer places within reach; the darkest areas can reach none at all."
#| fig-height: 6

acc_base() %>%
  addPolygons(data = acc_map, fillColor = ~pal_cum(places_30),
              fillOpacity = 0.8, color = "white", weight = 0.3,
              label = acc_lab) %>%
  addLegend(pal = pal_cum, values = acc_map$places_30,
            title = "Places within<br>30 minutes",
            position = "bottomright", opacity = 0.85)
```
